...
| `check:generated:executor` 脚本 | Go 1.26.5+ | 临时目录重生成并 byte compare Executor 生成物；generated models/strict server 随变更提交，纳入新鲜度检查 | experimental | Executor 生成配置与脚本 |
...
# Go 代码生成
pnpm --filter @tokenmp/contracts generate:executor:go # 生成 Executor generated models/strict server；生成物须随变更提交

# 生成物新鲜度检查
pnpm --filter @tokenmp/contracts check:generated
pnpm --filter @tokenmp/contracts check:generated:executor # generated models/strict server 随变更提交，纳入新鲜度检查

```

- 最小验证：`lint`（YAML 基本结构 + 禁止内部术语）、`typecheck`（跨文件 operationId 唯一性 + $ref 解析）、`test`（Node test runner 契约测试）、`build`（复制到 dist）。
- 契约测试：`tests/openapi-auth-v1.test.mjs`、`tests/openapi-config-v1.test.mjs`。
...
- **DO NOT** 手动编辑 `services/auth/internal/contract/authv1/{models,server}.gen.go` 或 `services/executor/internal/contract/executorv1/{models,server}.gen.go`——通过 `generate.sh` / `generate-executor.sh` 重生成。
- **DO NOT** 将 experimental Executor 生成配置/脚本误述为运行时服务行为——generated models/strict server 已生成并提交，供 adapter 与测试使用，但运行时 `main` 未注册公开业务路由，strict SSE 仅为 generated capability；CI 以现有 `go-auth` job 中必经的新鲜度门禁执行，无独立 Executor CI job/运行时路由。
...
- 适用范围：当前为 Auth；Executor generated models/strict server 随变更提交，其新鲜度与路由一致性同样适用。
