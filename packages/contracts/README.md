...
Executor 生成配置/脚本、generated models/strict server 与 transport adapter（Foundation `New()` 501 skeleton + DI `NewNonStream` non-stream adapter）已实施；生成物随变更提交。`check:generated:executor` 在现有 `go-auth` CI job 中作为必经的新鲜度门禁。generated handler 与 adapter（含 normalizer/renderer/strict options/raw-body capture，均模块内）现经 `services/executor/internal/composition` 接入 Executor runtime：runtime `main` 注册全部生成的 7 条路由——匿名 `GET`/`HEAD /healthz`、鉴权 non-stream `/v1/chat/completions`·`/v1/messages` 执行、鉴权 `/v1/models`·`/v1/responses` 返回 501，鉴权 `/v1/images/generations` 为 legacy completion-only non-stream 执行；Chat/Messages `stream:true` SSE 也已 runtime 启用。Images 共享 `internal/imagecontract` strict legacy boundary：default `url`、16 MiB wire/10 MiB item/12 MiB aggregate、URL/streaming base64、usage/extensions/revised-prompt，所有终态 `Cache-Control: no-store`；不支持 GPT Image 特有参数或 usage quota。契约形状与媒体安全边界：有限 request 对象关闭（`additionalProperties:false`），free-form JSON/JSON Schema 字段开放透传，成功 response schema 保持可扩展；image URL 仅接受 bounded HTTPS 或 `data:image` base64，`media_type` 枚举、base64 须标准 padded 且解码非空，详见 `AGENTS.md`。
