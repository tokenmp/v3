...
- Executor 架构设计基线（Foundation、Config compiler/immutable snapshot、routing、Adapter Engine、retry State、两个 non-stream SDK adapter、内部 non-stream Runner、non-stream HTTP transport 层与 phase 7.7 config source 前置 `internal/configsource` 均已实施；Runner/transport 层/config 文件源只作模块内 composition，不提供 wire-attempt proof 或跨进程 exactly-once；runtime 公开路由、runtime normalizer/renderer/identity/facade/reservation composition、config source wiring/reload、durable idempotency/replay、remote resolver、`Retry-After` 解析与流处理未实施）：`executor/architecture.md`
