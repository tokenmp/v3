...
- Executor Phase 8.2 internal StreamDriver：已组合 `AttemptSession`、stream registry、sequence-indexed owned payload adapter、protocol-neutral Bridge、pre-commit-only retry、quota、safe attempt log/result 与 bounded cleanup；OpenAI semantic stream 可内部调用。未接 HTTP transport/composition/runtime，`stream:true` 仍 501；不声称 HTTP atomicity/wire proof/public-provider E2E。按阶段表下一项为 Anthropic stream SDK，之后才是 HTTP SSE transport：`executor/architecture.md`、`executor/testing-strategy.md`
