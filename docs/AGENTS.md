...
- Executor Phase 8.2/9 internal streaming：`StreamDriver` 现可内部使用 OpenAI semantic stream 与 official Anthropic Messages `StreamClient`；后者 secure opening、adapter-owned bounded SSE parser/state 支持 text/thinking/tool/signature/ping/error/usage/message_stop，signature 仅 canonical payload 下游传递、native error classification first-wins。未接 HTTP transport/composition/runtime，`stream:true` 仍 501；不声称 HTTP atomicity/wire proof/public-provider E2E。下一阶段为 HTTP SSE transport，并在 composition 注册 OpenAI/Anthropic stream clients：`executor/architecture.md`、`executor/testing-strategy.md`
