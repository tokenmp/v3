...
- Executor Phase 8.1 OpenAI stream SDK：foundation 的 `AttemptPreparer`/`PreparedCall`/single-use `AttemptSession` 与 protocol-neutral `internal/streaming` Bridge 之外，shared `sdk.StreamClient`/`StreamSource`/`StreamEvent` 和 official OpenAI Chat `NewStreaming` internal adapter 已实施（Sequence + safe Meta + owned canonical Data、retry=0/no redirect/per-call auth/opening 2xx metadata、strict chunk parser/classifier、bounded no-raw exact `[DONE]` observer、Close/cancel）。未接 `AttemptSession` scoped-secret integration、attempt/retry/quota、Bridge payload adapter、transport/composition，`stream:true` 仍 501；不声称 HTTP atomicity/wire proof/public-provider E2E。下一阶段 stream-driver orchestration，随后 Anthropic streaming：`executor/architecture.md`、`executor/testing-strategy.md`
