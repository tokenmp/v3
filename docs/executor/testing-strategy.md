...
- public/provider E2E streaming、HTTP atomicity 或 wire-attempt proof；下一阶段为 Anthropic stream SDK，之后是 HTTP SSE transport。
...
| Streaming | **Phase 8.2 已实施（internal StreamDriver + race tests）**：`StreamDriver` 组合 `AttemptSession`、独立 stream registry、sequence-indexed owned-payload adapter 与 Bridge；测试覆盖 pre-commit-only retry、一次 Reserve/唯一 Finalize 或 Release、安全 attempt log/result、bounded cleanup、source sequence、payload bound 与 post-commit no-retry；OpenAI semantic stream 可内部调用。未接 HTTP transport/composition/runtime，`stream:true` 仍 501，不是 public/provider E2E、HTTP atomicity 或 wire proof。下一阶段 Anthropic stream SDK，之后 HTTP SSE transport。 |
