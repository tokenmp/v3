...
- 状态：测试设计基线；Foundation、Config compiler/snapshot、routing、Phase 5 Adapter Engine、Phase 6 non-stream SDK adapters、Phase 7 retry State、内部 non-stream Runner、non-stream HTTP transport 层、模块内 strict secret-free config 文件源 `internal/configsource`、credential/identity env 与 user-authorized runtime composition（`internal/composition`）的测试均已实施；runtime composition-level route conformance（枚举 OpenAPI 契约全部 operation，经全包装 handler 断言匿名/鉴权状态）与 process binary test（实际进程启动：health、unauth chat、鉴权空配置 chat 404 与 501 route、invalid 配置证明未 bind 监听器）已落地；Phase 8.2 internal StreamDriver tests 已实施（Phase 10 已将 stream capability 接入 runtime）；Phase 13 models catalog tests 已实施：`internal/modelcatalog` 的 capability/thinking 映射单元测试与 `internal/modelcatalogfacade` 的 catalog listing/quarantine/principal/snapshot/race 测试
...
durable idempotency/replay、remote quota/credential resolver、live provider E2E、持续 fuzz、性能与 Docker 仍是后续设计。Phase 15 Retry-After parsing tests 已实施。Phase 17 outbound Retry-After tests 已实施。

## Phase 10 HTTP streaming（已实施测试范围）

`internal/transport/executorv1api` 的 hybrid/plain generated handler 与 strict non-stream handler 共同覆盖 Chat/Messages：structural stream detection 后，`stream:false` 继续 strict path，`stream:true` 一次完整 normalization 后进入 stream facade/Driver。测试覆盖 auth 在 capture/body read 前、pre-commit 协议 JSON error、commit 后没有 JSON fallback、flushing SSE sink、256 KiB canonical event bound、OpenAI `data:`/唯一 `[DONE]` 与 Anthropic `event:`/`data:`（无 `[DONE]`）。composition tests 验证 completion 与 stream registry 成对注册，route conformance/process tests 验证 authenticated empty-config stream Chat/Messages 为 pre-commit JSON 404。CI race 显式覆盖 `./internal/stream/...` 与 `./internal/streamfacade/...`。不测试或不声称 HTTP atomicity、wire-attempt proof、跨进程 exactly-once 或 live provider E2E；Responses non-stream+stream 已 runtime 启用；Images legacy non-stream 已执行。
...
`Engine.MapResponse` 的 tests 断言 compiler-established order、所有填充维度的 AND / 同一维度 alternatives 的 OR、空 matcher fail-closed、matched rule ID 与固定安全 default；输入为 classified metadata，不读取或回显上游正文。这些场景已由 Phase 10+ 的 runtime HTTP pipeline 消费。
...
### 13.1 OpenAI SDK（Chat non-stream、内部 stream 与 Phase 11 Images completion-only runtime 已实施）
...
- Phase 11 legacy Images `Images.Generate`: completion-only non-stream registry/composition/transport runtime；facade 精确委托 Runner，Runner 一次 quota reservation 并按冻结策略 retry。call-local HTTPS target/model/opaque secret、retry=0/no redirects、唯一 Bearer auth；shared `internal/imagecontract` strict legacy request allowlist（不支持 GPT Image 特有参数）、缺省且 wire/renderer 显式 `url`、strict HTTPS URL/streaming standard padded base64/usage/extensions/revised-prompt validation、response wire 16 MiB、single decoded base64 item 10 MiB、aggregate 12 MiB、所有终态 no-store；TLS/race tests 与本地 fuzz 覆盖该边界。

超出当前部署范围：GPT Image 特有参数与 usage quota。
...
超出当前部署范围：public/provider E2E、HTTP atomicity/wire-attempt proof、durable idempotency/replay、remote quota/credential resolver。`Retry-After` parsing 已由 Phase 15 实施，outbound `Retry-After` header 已由 Phase 17 实施。
...
### 14.4 Phase 13 Models catalog tests

`internal/modelcatalog` 单元测试覆盖：

| 维度 | 已覆盖断言 |
|---|---|
| `MapCapabilities` | Chat→`text`、Tools→`function_calling`、Vision→`vision`、Thinking→`thinking`、Images→`image`；streaming/messages/responses 省略；多 capability 排序；空/nil 输入返回 nil |
| `MapThinking` | unsupported 返回 nil；supported 无 budget；supported 含 MinBudgetToken/MaxBudgetToken；零 budget 省略；单 effort level |
| `effortLevels` | none→max 全范围；medium→high 子范围；单 level；invalid 回退单 default |

`internal/modelcatalogfacade` 测试覆盖：

| 维度 | 已覆盖断言 |
|---|---|
| 正常 listing | enabled model 出现、按 ID 排序、capabilities 映射正确、created timestamp 来自 snapshot |
| Quarantine | active quarantine 排除 model；expired quarantine 包含 model；quarantine read error fail-closed `ErrQuarantineUnavailable` |
| Route 过滤 | 无 enabled route 的 model 不出现；disabled route 的 model 不出现 |
| Principal | 空/非法 Principal 返回 `ErrUnauthenticated`；active service/admin 通过；disabled/其他 role 拒绝 |
| 依赖 | nil store/nil quarantine 返回 `ErrMisconfigured`；空 store（无 published snapshot）返回 `ErrNoSnapshot` |
| 空配置 | 无 models/routes 返回空列表 |
| Thinking | thinking model 映射 ThinkingConfig（effort levels、budget tokens）；非 thinking model Thinking 为 nil |
| Race | 20 并发 ListModels 调用无 data race |
...
### 14.7 Phase 17 Outbound Retry-After tests

`execution.applyRetryAfter` 单元测试覆盖：

| 维度 | 已覆盖断言 |
|---|---|
| 429 + Retry-After 30s | `RetryAfterSeconds` = 30 |
| 529 + Retry-After 60s | `RetryAfterSeconds` = 60 |
| 429 无 Retry-After | `RetryAfterSeconds` = 0 |
| 500 + Retry-After 忽略 | `RetryAfterSeconds` = 0（非 429/529 不设置） |
| 429 + sub-second | `RetryAfterSeconds` = 0（< 1s 不设置） |
| 429 + 超大值 clamp | `RetryAfterSeconds` = 300（clamp 至 `HardMaxRetryAfter`） |
| nil classified | `RetryAfterSeconds` = 0 |
| nil mapped | `RetryAfterSeconds` = 0 |

`adapter.Engine.MapResponse` 单元测试覆盖：

| 维度 | 已覆盖断言 |
|---|---|
| 空 adapter + 429 upstream | `RetryAfterSeconds` = 0（MapResponse 不设置该字段） |
| matching rule + 429 | `RetryAfterSeconds` = 0 |
| default fallback | `RetryAfterSeconds` = 0 |

Transport renderer `renderer_test.go` 覆盖：

| 维度 | 已覆盖断言 |
|---|---|
| Chat 429 + RA 30s | `Retry-After: 30` header |
| Chat 429 无 RA | 无 `Retry-After` header |
| Chat 529 + RA 60s | chat maps 529→502，无 `Retry-After` header |
| Chat 529 无 RA | 无 `Retry-After` header |
| Message 429 + RA 30s | `Retry-After: 30` header |
| Message 529 + RA 60s | `Retry-After: 60` header（Anthropic 保留 529） |
| 非 rate-limit status + RA field | 无 `Retry-After` header |
| RA < 1s | 无 `Retry-After` header |
| RA > 300 clamp | `Retry-After: 300` |
| Image 429 + RA | `Retry-After` header |
| Image 429 无 RA | 无 `Retry-After` header |
| Response 429 + RA | `Retry-After` header |
...
现有 `go-auth` CI job 已运行 `check:generated:executor`、generated transport/route conformance race tests，以及从 `services/executor` 执行的 `go test -race -count=1 ./internal/adapter/... ./internal/snapshot/... ./internal/routing/... ./internal/execution/... ./internal/requestlog/... ./internal/quota/... ./internal/sdk/... ./internal/configsource/... ./internal/credentialenv/... ./internal/identityenv/... ./internal/quarantinebridge/... ./internal/imagecontract/... ./internal/nonstream/... ./internal/nonstreamfacade/... ./internal/authcontext/... ./internal/requestid/... ./internal/stream/... ./internal/streamfacade/... ./internal/streaming/... ./internal/modelcatalog/... ./internal/modelcatalogfacade/... ./internal/responsecontract/... ./internal/composition/... ./internal/config/... ./internal/app/... ./cmd/executor/...`。后者是 compiler/snapshot/routing/internal Runner/adapter SDK/config source/models catalog/composition 的最小 race 门禁；`./internal/execution/...` 覆盖 internal Runner、registry、terminal 与 request-local retry State 的单元、invariant 与 race tests，`./internal/requestlog/...` 与 `./internal/quota/...` 覆盖 Runner 使用的日志和 quota ports，`./internal/configsource/...` 覆盖 strict secret-free 文件源与 initial generation=1 bootstrap，`./internal/config/...` 覆盖 composition-aware 必填 env 校验，`./internal/app/...` 覆盖注入 handler 创建 server，`./cmd/executor/...` 覆盖实际进程启动与 invalid 配置 fail-before-listen，`./internal/modelcatalog/...` 与 `./internal/modelcatalogfacade/...` 覆盖 catalog boundary/facade 的 capability 映射、quarantine 过滤、Principal 校验与 race。`./internal/sdk/...` 自动涵盖 OpenAI（包括 completion-only legacy Images）与 Anthropic adapter packages，包括本地 TLS `httptest`、race 和 fuzz seed tests；独立 shared validator `./internal/imagecontract/...` 亦被显式 race 覆盖；持续 fuzz 不加入 CI。`./internal/composition/...` 覆盖 runtime composition wiring，包括枚举 OpenAPI 契约全部 operation、经全包装 `AuthMiddleware(CaptureRawBody(...))` handler 断言匿名/鉴权状态的路由一致性，与 process binary test（health、unauth chat 401、鉴权空配置 chat 404 与 501 route、invalid 配置未 bind 监听器）。`./internal/quarantinebridge/...`、`./internal/modelcatalog/...` 与 `./internal/modelcatalogfacade/...` 被显式列出是因为它们与 `./internal/routing/...` 是独立 package，routing 的 race pattern 不会自动测试它们，必须单独加入包列表才会被覆盖。SDK HTTP tests 仅访问进程内本地 `httptest.NewTLSServer`，不访问网络、数据库或真实 Provider。该命令不运行数据库、live provider 或远端 request pipeline，也不构成独立 Executor CI job；独立 Executor CI job 与远端/真实 provider 集成超出当前部署范围。
