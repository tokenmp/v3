...
- 状态：测试设计基线；Foundation、Config compiler/snapshot、routing、Phase 5 Adapter Engine、Phase 6 non-stream SDK adapters、Phase 7 retry State、内部 non-stream Runner、non-stream HTTP transport 层、模块内 strict secret-free config 文件源 `internal/configsource`、credential/identity env 与 user-authorized runtime composition（`internal/composition`）的测试均已实施；runtime composition-level route conformance（枚举 OpenAPI 契约全部 operation，经全包装 handler 断言匿名/鉴权状态）与 process binary test（实际进程启动：health、unauth chat、鉴权空配置 chat 404 与 501 route、invalid 配置证明未 bind 监听器）已落地；Phase 8.1 OpenAI internal stream SDK tests 已实施（尚未 runtime 接线）
...
- shared `sdk.StreamClient`/`StreamSource`/`StreamEvent` semantic-stream boundary；`StreamEvent` has monotonic `Sequence`, safe metadata and adapter-owned canonical `Data`;
...
- strict chunk parser/classifier; bounded no-raw SSE observer proves exactly one terminal `[DONE]`; `Close` and caller cancellation release in-flight reads;
...
- `AttemptSession` scoped-secret integration、attempt/retry/quota orchestration、Bridge payload adapter；
- runtime HTTP normalizer/renderer/identity 与公开路由；schema-valid `stream:true` 仍为 501；
- durable idempotency/replay、remote quota/credential resolver、`Retry-After` parsing；
- public/provider E2E streaming、HTTP atomicity 或 wire-attempt proof；下一阶段 stream-driver orchestration。
...
| Streaming | **Phase 8.1 已实施（internal SDK adapter + foundation race tests）**：`internal/streaming` Bridge 的 logical Sink commit、TTFT/idle/lifetime/cancel precedence、usage/outcome、bounded event/source/sink behavior and sink-uncertain no-retry；shared `sdk.StreamClient`/`StreamSource`/`StreamEvent` 与 official OpenAI Chat `NewStreaming` adapter 已提供 strict chunk parsing/classification、bounded no-raw exact-`[DONE]` observation、Close/cancel。尚未接 `AttemptSession` scoped-secret integration、attempt/retry/quota、Bridge payload adapter、transport/composition；`stream:true` 仍 501，不是 public/provider E2E、HTTP atomicity 或 wire proof。 |
