...
- `FuzzAnthropicSSEParser` 为本地 fuzz smoke；CI 完整模块 `go test -race ./...` 自动覆盖该 package。
...
现有 `go-auth` CI job 已运行 `check:generated:executor`，并从 `services/executor` 执行完整模块 `go test -race ./...`。该命令不依赖手工 package allowlist，因此包括 compiler/snapshot/routing/internal Runner、adapter SDK、config source、catalog、runtime composition，以及未来新增的 Executor package 都会自动纳入 race 覆盖。它覆盖 internal Runner、registry、terminal、request-local retry State、requestlog/quota ports、strict secret-free config source、composition route conformance 与 process binary test；SDK HTTP tests 仅访问进程内本地 `httptest.NewTLSServer`。该命令不运行数据库、live provider 或远端 request pipeline，也不构成独立 Executor CI job；独立 Executor CI job 与远端/真实 provider 集成超出当前部署范围。
...
| Config compiler/snapshot | **已实施（模块内 + 完整模块 CI race 门禁）**：compiler、immutable generation-aware Store、三份 fixture 的严格解码/真实编译/发布测试、C01–C27 覆盖和 `FuzzCompile` smoke；现有 `go-auth` job 运行 `go test -race ./...`，不依赖手工 package allowlist，覆盖 compiler/snapshot/routing/internal Runner/adapter SDK/config source/runtime composition（含 route conformance 与 process binary）及未来新增的 Executor package；不运行 DB、真实 Provider 或远端 request pipeline，SDK HTTP tests 仅使用本地 TLS `httptest` server；默认值已与架构基线对齐 |
...
| Quarantine bridge | **已实施（经 composition 接入 runtime facade）**：`internal/quarantinebridge` 把 runtime quarantine port/state 适配为 routing quarantine reader（每维度独立带前缀 runtime target，跨维度重复 ID 不冲突）与 fail-closed 错误处理（not-found→`routing.ErrNotFound`、context 取消归一为裸 sentinel、其余失败（含 nil/typed-nil port）→`routing.ErrQuarantineUnavailable`）；是由完整模块 `go test -race ./...` 自动覆盖的独立 package；经 `internal/composition` 接入 runtime facade，本身不拥有写入能力 |
