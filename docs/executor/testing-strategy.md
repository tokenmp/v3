...
- 状态：测试设计基线；Foundation、Config compiler/snapshot、routing、Phase 5 Adapter Engine、Phase 6 non-stream SDK adapters、Phase 7 retry State、内部 non-stream Runner、non-stream HTTP transport 层、模块内 strict secret-free config 文件源 `internal/configsource`、credential/identity env 与 user-authorized runtime composition（`internal/composition`）的测试均已实施；runtime composition-level route conformance（枚举 OpenAPI 契约全部 operation，经全包装 handler 断言匿名/鉴权状态）与 process binary test（实际进程启动：health、unauth chat、鉴权空配置 chat 404 与 501 route、invalid 配置证明未 bind 监听器）已落地；Phase 8.2 internal StreamDriver tests 已实施（尚未 runtime 接线）
- 适用范围：TokenMP v3 `services/executor`
- 架构来源：`architecture.md`
- API 契约：`packages/contracts/openapi/executor/v1.yaml`

Foundation 已实施的测试范围：运行时配置校验、`GET` / `HEAD /healthz`、优雅关闭、config、identity、quota、request log 和 runtime 端口的 Mock/InMemory contract 测试，以及 generated transport 的生成物标记检查（`freshness_test.go`）与路由契约一致性测试（`internal/server/contract_test.go`）；其中 quota 覆盖 reservation 从 `reserved` 到 `finalized` 或 `released` 的唯一终态、同终态幂等和相反终态冲突。Config compiler/snapshot 阶段已实施 `snapshot.Compile` → `adapter.Compile`、`internal/adapter/compiler_test.go`、`internal/snapshot/compiler_test.go`、`store_test.go`（无快照、发布深拷贝、旧 revision 稳定、被拒发布保留 last known good、并发读者/发布者）和 fixture tests。三份 fixture 以 `DisallowUnknownFields` 严格解码、secret 扫描、枚举/duration/round-trip/HTTPS BaseURL 检查后实际编译和发布；BaseURL 现在还拒绝 query/fragment/userinfo（`RawQuery`/`ForceQuery`/`Fragment`），错误仅命名公开 provider key，不泄露 URL 或内容（`TestCompileBaseURLRejectsQueryFragment`/`TestCompileBaseURLAcceptsCleanHTTPS`）。测试还覆盖 xfyun `503→429`、thinking 至多 `medium`，以及 Anthropic `529→429`。C01–C27 相关 compiler/snapshot 安全、默认值、immutability 与 determinism 测试均已实施，详见第 4 节；`FuzzCompile` 和 `FuzzParseSelector` 已作为 fuzz smoke 实施。routing tests 覆盖 strict selector、revision-pinned deterministic Resolver/Plan、legacy credential synthesis、candidate scopes、四维 fail-closed quarantine、immutable private fallback universe 与并发 resolve；公开 Candidate/Plan 不含 credential ref 或 secret。模块内 `internal/quarantinebridge` 测试已实施：覆盖把 runtime quarantine port/state 适配为 routing quarantine reader（每个 routing quarantine 维度映射到独立带前缀 runtime target，跨维度重复 ID 不冲突）与 fail-closed 错误处理（not-found 保留为 `routing.ErrNotFound`、context 取消归一为裸 sentinel、其余读取失败（含 nil/typed-nil port）均以 `routing.ErrQuarantineUnavailable` 暴露）。该包现经 `internal/composition` 接入 runtime facade（request pipeline 的 routing quarantine reader），仍不本身拥有写入能力。模块内 config 文件源 `internal/configsource` 的测试已实施：覆盖 `LoadFile` 的 fixture 加载、blank path/trim、not found、symlink/目录/命名管/non-regular 拒绝、post-open `SameFile` TOCTOU 与 identity mismatch、1 MiB 上限与 stat-read 增长、空文件、重复/prototype-pollution key、深度/节点上限、top-level 非 object、unknown field、trailing data、严格 UTF-8、context cancel/deadline（stat-open/read-parse 边界）、sentinel 不泄露 path/content 且 non-wrapping、`ScanSecrets` lexical+semantic 双通道（JSON-escape/URL percent-encoding/JWT/forbidden key/value marker、CredentialRef 无误报、嵌套容器值后 key、malformed 不 panic、空输入）、`HasSecret`，以及 `CompileAndPublishInitial` 的 fixture bootstrap（initial generation=1、store view 一致）、meta mutation isolation、stale/已 bootstrap 拒绝、nil store、compile 失败、revision trim、pre-canceled ctx 不发布、原子并发与 `FuzzScanSecrets`/`FuzzScanSecretsNestedContainerState`/`FuzzParseConfigSentinels` fuzz smoke。该包现经 `internal/composition` 在启动时接入（`CompileAndPublishInitial` 发布 initial generation=1）；未实现热重载 loop。模块内 `internal/credentialenv` 测试已实施：严格、bounded、secret-free `vault://` credential ref → `EXECUTOR_CREDENTIAL_*` env-name JSON map；每 attempt lookup 以观察 rotation；opaque-secret callback copy/clear；redacted formatting/safe sentinels；以及 `ValidateCompiled` 对 enabled authenticated route 的 exact enabled-ref usable-map startup validation。map JSON 与 secret env values 不提交；该包现经 `internal/composition` 接入 runtime，在启动预检中要求 enabled route 的 credential refs 与可用 mapping 精确一致。identity env 已作为模块内能力实施：`internal/identityenv` 严格验证非 secret `EXECUTOR_IDENTITY_MAP_JSON` entry ID → subject/key_id/role/status/`EXECUTOR_API_KEY_*`，每次认证重读 API-key 环境变量以支持 rotation；`executorv1api.AuthMiddleware` 在 `CaptureRawBody` 外层、body read/decode 前保护全部 `/v1`，以协议原生无泄漏 401 fail closed，并保留 `/healthz` 匿名。二者现经 `internal/composition` 接入 `main`、app 与公开 runtime routes；未做 Auth JWT。Phase 5 Adapter Engine tests cover strict JSON, every finite DSL action, literal-only header/query, continued `ValueRef` rejection, model-bounded thinking, safe response mapping, atomicity/mutation isolation, race, and fuzz. Response mapping is consumed by the internal Runner for classified non-stream failures, and is wired into runtime via Runner/composition. 已实施的 OpenAI Chat 与 Anthropic Messages non-stream SDK adapter tests 均使用 TLS `httptest`，覆盖 per-call HTTPS target/model/secret、SDK retry=0、no redirects、严格请求/响应 validator、安全 injection 与唯一 provider auth、safe attempt observer、success metadata，以及 timeout/transport/protocol/HTTP safe classification；Anthropic 还覆盖 `WithoutEnvironmentDefaults`、`/v1/messages`、唯一 `x-api-key` 和固定 `anthropic-version`、环境 header/target 隔离、thinking authority、tools/vision、`529 overloaded_error→unavailable` 与 fixture `529→429` mapping，并以 `FuzzDecodeMessageParams` 和 `FuzzValidateMessageResponse` 执行参数/响应 validator fuzz smoke。两个 adapter 现经内部 non-stream Runner 与 `internal/composition` 接入 runtime routing（chat/messages 执行）。内部 non-stream Runner 的 Mock/InMemory/fake tests 已实施：覆盖 Resolver/Plan owner binding、每 attempt pure Prepare/credential resolve、Engine/registry/auth compatibility、frozen retry policy、per-attempt timeout、one Reserve/safe terminalizer、mapped classified failure 和 safe execution events。它不证明 wire attempt 或跨进程 exactly-once。runtime composition-level route conformance（`internal/composition/route_conformance_test.go` 枚举 OpenAPI 契约全部 operation，经全包装 `AuthMiddleware(CaptureRawBody(generated))` handler 断言匿名/鉴权状态）与 process binary test（`cmd/executor/main_unix_test.go`：实际进程 health、unauth chat 401、鉴权空配置 chat 404 与 501 route、invalid 配置证明未 bind 监听器）已实施。Phase 8.1 OpenAI stream SDK is implemented internally. Alongside the shared `AttemptPreparer`/opaque `PreparedCall`/single-use `AttemptSession` and the pure-Go protocol-neutral `internal/streaming` Bridge, shared `sdk.StreamClient`/`StreamSource`/`StreamEvent` now provide a semantic-stream boundary: each event has a monotonic `Sequence`, bounded safe metadata, and adapter-owned canonical `Data` capped at 256 KiB. The official OpenAI Chat adapter opens one stream through `openai-go` Chat Completions `NewStreaming`, with retry=0, redirects disabled, per-call Bearer authentication, and safe opening 2xx status/request-ID metadata. It strictly parses/classifies chunks at the same 256 KiB cap; its bounded no-raw SSE observer proves exactly one terminal `[DONE]`; and `Close`/cancellation release an in-flight stream read. The exact SDK/protocol registry holds complete and stream capabilities independently; `PreflightStream` and `AttemptSession.ExecuteStream` provide scoped-secret one-attempt opening and close a returned source on opening failure. Phase 8.2 internal `StreamDriver` tests now cover composition of `AttemptSession`, exact stream registry, sequence-indexed owned-payload source/sink adapter and Bridge; request-local pre-commit-only retry, one quota Reserve plus unique Finalize/Release, safe attempt log/result and bounded cleanup; OpenAI semantic stream is internally callable. They also cover source sequence, payload bounds, post-commit no-retry and cleanup. There is still no HTTP transport, composition or runtime consumer: schema-valid `stream:true` remains 501, with no HTTP atomicity, wire-attempt proof or public/provider E2E claim. Phase 9 Anthropic internal StreamClient is now implemented; the next phase is HTTP SSE transport plus composition RegisterStream for OpenAI and Anthropic.

 durable idempotency/replay、remote quota/credential resolver、`Retry-After` parsing、SDK/provider stream、transport/composition stream 接线、集成、持续 fuzz、性能、Docker 与 CI 测试仍是后续设计。retry State 的单元、invariant 与 race tests 已实施；它本身仍不接 runtime。non-stream HTTP transport 层（`internal/transport/executorv1api`）的 generated-handler component tests 已实施：`adapter_integration_test.go` 将 DI `NewNonStream` adapter 接入 generated `NewStrictHandler`，经 `CaptureRawBody` 中间件，以 kin-openapi 校验每条 non-stream path 的响应；`body_test.go`、`normalizer_test.go`、`renderer_test.go`、`strictoptions_test.go`、`requestid_test.go` 覆盖 raw-body 捕获（2 MiB 上限与超限/不可读 400）、strict normalizer（镜像 `additionalProperties:false`、thinking/媒体/JSON 边界、streaming 拒绝）、protocol renderer（bounded local contract check、协议原生错误/成功 body、`request_id` 仅来自脱敏 SDK metadata）、`SafeStrictOptions`（不泄漏 error handler、context cancel/deadline 不写）与 trusted request-id 生成。该层现经 `internal/composition` 接入 identity/runtime config/facade/reservation 与公开路由。
...
注：`internal/server/contract_test.go` 以 kin-openapi 加载契约（对 `description`/`nullable` 与 `$ref` 的 OAS 3.0 同位词法使用 `AllowExtraSiblingFields` 宽容，与项目契约验证器一致），遍历 generated `Handler` 的 Chi 路由，与契约 method+path 双向比较（7 条路由）。该测试证明生成路由与契约一致。runtime composition-level route conformance（`internal/composition/route_conformance_test.go`）进一步枚举契约全部 operation，经 runtime 全包装 handler（`AuthMiddleware(CaptureRawBody(generated))`）断言每条 operation 的匿名与鉴权状态，不以 chi.Walk 依赖 wrapper 实现。
...
内部 non-stream Runner 已以 Mock/InMemory/fake 覆盖其模块内组合路径：Resolver-owned Plan binding、preflight、per-attempt credential resolution、Engine、registry、official SDK auth compatibility、frozen retry policy、per-attempt timeout、一次 Reserve、safe terminalizer、mapped failure 与 safe execution event。它经 runtime composition（identity/runtime config/facade/reservation 接入公开 server）接入公开 runtime route，但不构成 durable idempotency/replay、wire proof 或跨进程 exactly-once。non-stream HTTP transport 层（normalizer/renderer/strict options/raw-body capture/DI adapter）的 component tests 已实施（见下），并经 composition 接入 runtime。composition-level route conformance test（`internal/composition/route_conformance_test.go`）枚举 OpenAPI 契约全部 operation，经全包装 handler 断言匿名/鉴权状态；process binary test（`cmd/executor/main_unix_test.go`）验证实际进程启动与 invalid 配置未 bind 监听器。
...
## 9. Retry、Attempt Budget 与内部 non-stream Runner 测试（部分已实施；经 composition 接入 runtime）
...
protocol renderer 与 `SafeStrictOptions` 已实施模块内 component tests 覆盖 OpenAI/Anthropic 错误形状、bounded local contract check、context cancel/deadline 不写响应与无泄漏；该层现经 `internal/composition` 接入 identity/runtime config/facade/reservation 与公开路由。
...
- shared `sdk.StreamClient`/`StreamSource`/`StreamEvent` semantic-stream boundary；`StreamEvent` has monotonic `Sequence`, safe metadata and adapter-owned canonical `Data` capped at 256 KiB;
- internal OpenAI Chat `NewStreaming` adapter: exactly one opening call, retry=0, no redirects, per-call unique Bearer auth, and only safe opening 2xx status/request-ID metadata;
- strict chunk parser/classifier; bounded no-raw SSE observer is aligned to the 256 KiB cap and proves exactly one terminal `[DONE]`; `Close` and caller cancellation release in-flight reads;
...
- HTTP transport/composition/runtime HTTP normalizer/renderer/identity 与公开路由；schema-valid `stream:true` 仍为 501；
...
Phase 9 streaming tests 已实施：

- secure opening 的 HTTPS/path prefix、environment isolation、authoritative model/thinking、retry=0/no redirects、sole `x-api-key`、fixed version、SSE Accept、single request、safe opening metadata 与 cleanup/cancellation；
- adapter-owned bounded SSE parser/state（不依赖 SDK event object）对 `message_start`、text/thinking/tool delta、thinking signature、ping、native error、usage、`message_stop` 的严格顺序、canonical payload/256 KiB bound、单调 source sequence、Close/concurrent Close 与 pump convergence；
...
仍待后续：HTTP SSE transport、composition `RegisterStream`、runtime/public provider E2E、HTTP atomicity/wire-attempt proof，以及 durable idempotency/replay、remote quota/credential resolver、`Retry-After` parsing。
...
### 13.4 Phase 8.2 internal StreamDriver（已实施测试矩阵）

`internal/execution/{stream_driver,stream_payload}_test.go` 覆盖内部组合，不启动 HTTP/SSE transport：

| 维度 | 已覆盖断言 |
|---|---|
| retry | opening 与 pre-commit classified native failure 可按冻结 policy 重试；commit 后失败绝不 retry；parent cancel 优先。 |
| quota / cleanup | 首次成功 preflight 后一次 Reserve；commit 前失败 Release；确认 usage 的 commit 后成功/失败 Finalize；unresolved cost Release；terminal failure 不误记 success；cleanup 使用 bounded cancel-independent context。 |
| log / result | 每 attempt 仅记录 safe metadata；成功、失败、terminalization failure 与 cancel 不把未确认 terminal 写成 success；result 仅在选定 quota terminal 确认后返回。 |
| sequence / payload | source `Sequence` 非零且严格递增；metadata 按 sequence 精确配对 owned canonical JSON payload；256 KiB/event 与 35-event pending upper bound；copy isolation、Flush staging、failure retention、Discard/Close 清理。 |
| bridge / lifetime | lifecycle buffer、semantic commit、post-commit no-retry、native error、TTFT/idle/lifetime/cancel precedence、Close/drain 不泄漏 read goroutine。 |

这些是 internal StreamDriver 的组件/race 覆盖，不等同于 HTTP atomicity、wire-attempt proof、公开 provider E2E 或 runtime stream 路由测试。
...
`internal/streaming` 已有 `FuzzBridgeRun`；它验证 foundation 的 bounded protocol-neutral lifecycle。OpenAI stream parser/classifier 的 fuzz target如后续添加，仅作本地按需运行，**不加入 CI**。
...
| Codegen | **已实施**：freshness + route/HTTP conformance（codegen/route conformance 本身不确立 runtime 路由注册或业务执行；runtime wiring 由单独实施的 composition-level route conformance test 验证全部 7 条 runtime 路由的匿名/鉴权状态，并由非流式 chat/messages 执行 composition 覆盖） |
| Config compiler/snapshot | **已实施（模块内 + 最小 CI race 门禁）**：compiler、immutable generation-aware Store、三份 fixture 的严格解码/真实编译/发布测试、C01–C27 覆盖和 `FuzzCompile` smoke；现有 `go-auth` job 运行 `go test -race -count=1 ./internal/adapter/... ./internal/snapshot/... ./internal/routing/... ./internal/execution/... ./internal/requestlog/... ./internal/quota/... ./internal/sdk/... ./internal/configsource/... ./internal/credentialenv/... ./internal/identityenv/... ./internal/quarantinebridge/... ./internal/nonstream/... ./internal/nonstreamfacade/... ./internal/authcontext/... ./internal/requestid/... ./internal/streaming/... ./internal/composition/... ./internal/config/... ./internal/app/... ./cmd/executor/...`，覆盖 compiler/snapshot/routing/internal Runner/adapter SDK/config source/runtime composition（含 route conformance 与 process binary）；不运行 DB、真实 Provider 或远端 request pipeline，runtime config source wiring 与公开 runtime pipeline 已由 composition-level route conformance test（全部 7 条路由匿名/鉴权状态）与 process binary test（实际进程启动、invalid 配置未 bind 监听器）覆盖；SDK HTTP tests 仅使用本地 TLS `httptest` server；默认值已与架构基线对齐。`./internal/quarantinebridge/...` 被显式列出是因为它与 `./internal/routing/...` 是独立 package，routing 的 race pattern 不会自动测试它 |
...
| Streaming | **Phase 8.2/9 已实施（internal StreamDriver + OpenAI/Anthropic SDK tests）**：`StreamDriver` 可内部使用 OpenAI 与 Anthropic `StreamClient`；Anthropic tests 覆盖 secure opening、bounded adapter-owned SSE parser/state、text/thinking/tool/signature/ping/error/usage/message_stop、payload/signature redaction、native-error first-wins、source sequence、Close 与 local fuzz smoke。未接 HTTP transport/composition/runtime，`stream:true` 仍 501，不是 public/provider E2E、HTTP atomicity 或 wire proof。下一阶段 HTTP SSE transport，并在 composition 为 OpenAI 与 Anthropic 注册 StreamClient。 |
| Anthropic streaming | **已实施（internal SDK）**：native SSE + thinking signature；HTTP SSE transport/composition/runtime 仍待后续 |
