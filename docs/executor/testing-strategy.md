...
`internal/streaming` 已有 `FuzzBridgeRun`；它验证 foundation 的 bounded protocol-neutral lifecycle，不解析 SSE 或证明 provider wire behavior。
...
| Streaming | **部分已实施（foundation race/fuzz）**：`internal/streaming` Bridge 的 logical Sink commit、TTFT/idle/lifetime/cancel precedence、usage/outcome、bounded event/source/sink behavior and sink-uncertain no-retry；尚无 SDK/provider stream、SSE parser、attempt/retry/quota/transport/composition 接线或 HTTP/wire proof |
