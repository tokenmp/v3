...
Foundation 不连接数据库，使用 Mock/InMemory ports。Phase 10 已将 OpenAI Chat 与 Anthropic Messages `stream:true` 接入 runtime：generated hybrid plain+strict dispatch 保持 `stream:false` strict/non-stream 路径，stream path 一次完整 normalization 后走 transport-neutral facade、StreamDriver 和 exact SDK stream registry；`AuthMiddleware(CaptureRawBody(...))` 保持 auth-before-body-read。pre-commit 失败是协议 JSON，SSE commit 后绝无 JSON fallback；OpenAI `[DONE]`、Anthropic native event framing，canonical payload 256 KiB。Driver 保持 pre-commit retry、一次 Reserve/唯一 terminal。Responses/Images 仍 501；不声称 HTTP atomicity、wire proof、跨进程 exactly-once 或 public/provider E2E。
