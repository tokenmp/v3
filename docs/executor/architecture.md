...
| 8.1 | `feat/executor-openai-stream-sdk` | **已实施（内部 SDK adapter，未 runtime 接线）**：shared `sdk.StreamClient`/`StreamSource`/`StreamEvent`（monotonic `Sequence`、safe metadata、adapter-owned canonical `Data`），以及 official OpenAI Chat `NewStreaming` adapter；retry=0、no redirects、per-call Bearer auth、safe opening 2xx metadata、strict chunk parser/classifier、bounded no-raw SSE observer 精确验证一个 `[DONE]`、`Close`/cancel。未接入 `AttemptSession` scoped-secret integration、attempt/retry/quota、Bridge payload adapter、transport 或 composition；`stream:true` 仍 501，不声称 HTTP atomicity、wire proof 或 public/provider E2E。下一阶段为 stream-driver orchestration；Anthropic streaming 后续实施。 |
