...
| `anthropic` | `github.com/anthropics/anthropic-sdk-go` v1.58.0 | Anthropic Messages non-stream 已实施；native streaming 及兼容 Provider 仍待后续 |
...
Phase 8.1 OpenAI stream SDK is implemented internally. Alongside the shared `AttemptPreparer`/opaque `PreparedCall`/single-use `AttemptSession` and the pure-Go protocol-neutral `internal/streaming` Bridge, shared `sdk.StreamClient`/`StreamSource`/`StreamEvent` now provide a semantic-stream boundary: each event has a monotonic `Sequence`, bounded safe metadata, and adapter-owned canonical `Data` capped at 256 KiB. The official OpenAI Chat adapter opens one stream through `openai-go` Chat Completions `NewStreaming`, with retry=0, redirects disabled, per-call Bearer authentication, and safe opening 2xx status/request-ID metadata. It strictly parses/classifies chunks at the same 256 KiB cap; its bounded no-raw SSE observer proves exactly one terminal `[DONE]`; and `Close`/cancellation release an in-flight stream read. A post-commit native error is a safe upstream-error outcome and is never sent to a Sink. The exact SDK/protocol registry holds complete and stream capabilities independently; `PreflightStream` and `AttemptSession.ExecuteStream` provide scoped-secret one-attempt opening and close a returned source on opening failure. Phase 8.2 adds the internal `execution.StreamDriver`: it composes `AttemptSession`, exact stream registry lookup, a sequence-indexed owned-payload source/sink adapter, and Bridge with request-local pre-commit-only retry, one quota Reserve plus a unique Finalize or Release, safe attempt log/result, and bounded cleanup. The OpenAI semantic stream can now be called internally. Source sequence stays strictly increasing and canonical payloads stay bounded; post-commit paths never retry. It still has no HTTP transport, composition, or runtime consumer, so schema-valid `stream:true` returns 501. It does not claim HTTP atomicity, wire-attempt proof, or public/provider E2E. Per the phase table, Anthropic stream SDK is next, followed by HTTP SSE transport.
