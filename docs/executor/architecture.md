...
| 7.7 | `feat/executor-runtime-composition` | **已实施（runtime composition）**：`internal/composition.Build(ctx,cfg,lookupEnv)` 在 `net.Listen` 之前组装 immutable snapshot store、`configsource.CompileAndPublishInitial` initial generation=1、credential env `NewFromJSON`+`ValidateCompiled`、identity env `NewFromEnv`、in-memory runtime/quarantinebridge/quota/execution log、精确 OpenAI/Anthropic completion SDK registry 与 OpenAI/Anthropic stream registry、Runner、facade、`NewNonStream` adapter、generated strict handler（`SafeStrictOptions`）、`AuthMiddleware(identity)(CaptureRawBody(generated))`；启动拒绝不受支持的 enabled SDK/protocol route。生成的 7 条路由均为运行时实际路由：匿名 health、鉴权 501 的 models/responses、鉴权执行的 chat/messages 与 completion-only non-stream Images。`config.Load` 要求 `EXECUTOR_CONFIG_FILE`/`EXECUTOR_CREDENTIAL_REF_MAP_JSON`/`EXECUTOR_IDENTITY_MAP_JSON`（错误 redacted）；`app.NewServer` 以注入 handler 创建 server、nil/typed-nil fail-closed。仍未实现热重载 loop、durable idempotency/replay、remote quota/credential resolver、`Retry-After` parsing、public/provider streaming driver 与 transport/composition stream 接线、Responses 执行、数据库、Docker。 |
...
| 14 | `feat/executor-request-logging` | Request/Attempt/Event 日志 |
| 15 | `build/executor-ci-docker` | Docker、CI 和集成验证 |
| 16 | 独立决策 | PostgreSQL/Redis/internal HTTP repositories |
