...
│   ├── streaming/              # 已实施 protocol-neutral Bridge/state/timer；SSE parser 与 provider adapters 待接线
...
Phase 8 streaming foundation is partially implemented: `internal/execution` now exposes shared `AttemptPreparer`/opaque `PreparedCall` and single-use `AttemptSession`; the session supplies an SDK call only to a synchronous scoped-secret callback and revokes the secret when that callback returns. The pure-Go, protocol-neutral `internal/streaming` Bridge is also implemented: bounded sanitized event metadata, pre-commit lifecycle buffering, a Sink commit gate, TTFT/idle/lifetime/cancellation precedence, bounded monotonic usage, terminal outcomes, and bounded source/sink behavior. A successful `Sink.Commit` means the complete logical batch was written and flushed **under the Sink contract**; it is not an HTTP-atomicity or wire-attempt proof. Sink uncertainty is terminal and never retried. These foundations are not yet connected to attempt/retry/quota orchestration, SDK streams, HTTP transport, or runtime composition: public `stream:true` remains rejected/501 as applicable to the existing transport path, and there is no real provider streaming. The next PR should add the SDK semantic-stream port and official adapters in OpenAI-first, Anthropic-second order, or add stream-driver orchestration after those ports exist.
...
| 8 | `feat/executor-streaming` | **部分已实施（foundation）**：共享 `AttemptPreparer`/opaque `PreparedCall`/single-use `AttemptSession`；session scoped-secret callback 后 revoke；纯 Go protocol-neutral `internal/streaming` Bridge 实施 bounded state/event metadata、logical Sink `Commit`（batch written+flushed contract）、TTFT/idle/lifetime/cancel、usage/outcome 与 sink-uncertain no-retry。未接入 attempt/retry/quota、SDK stream、transport 或 composition；`stream:true` 仍不可用，无真实 provider stream、HTTP atomicity 或 wire proof。下一 PR：SDK semantic stream port + OpenAI-first/Anthropic-second adapters，或其后的 stream-driver orchestration。 |
