...
开发初期不连接数据库。所有身份、配置、配额、日志和运行时状态均通过端口接口及 Mock/InMemory 实现提供。未来持久化实现不得改变核心执行流程。
...
每个 Repository 在进入持久化阶段前必须明确数据所有者。开发初期全部使用 Mock/InMemory 实现，不连接数据库。
...
Executor 契约已存在于 `packages/contracts/openapi/executor/v1.yaml`。仓库还预置了生成脚本和 `chi-server + strict-server` 配置，但 `services/executor/` 与生成产物尚不存在，因此 `check:generated:executor` 目前不是可执行的现有 CI 基线。
...
| 阶段 | 分支建议 | 内容 |
|---|---|---|
| 1 | `feat/executor-foundation` | 模块骨架、health、Mock repositories、最小 quota terminal 状态机 |
