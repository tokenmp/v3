...
**套餐操作**：
- 分配套餐（Modal：选套餐 + 到期时间；user 由详情页固定；默认按套餐 category 填充到期：daily=1、weekly=7、monthly=30、quarterly=90、yearly=365 天）
- 续费套餐（默认延长一个套餐周期，或显式设置新到期时间）
- 切换套餐（取消旧 user_plan，并创建新 plan 绑定；默认保留旧 user_plan 的 expires_at，只有管理员显式填新到期时间才覆盖；目标套餐必须同类型且最大总额度不能低于当前套餐；coding 只比较周期总额度，5小时/周额度是节流限制不代表套餐等级；token 比较 token_limit；不按价格硬拦，因为运营赠送套餐可 price=0 但额度更高；历史请求/账本不修改）
