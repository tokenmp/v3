...
- 切换套餐（取消旧 user_plan，并创建新 plan 绑定；目标套餐必须同类型且最大总额度不能低于当前套餐；coding 只比较周期总额度，5小时/周额度是节流限制不代表套餐等级；token 比较 token_limit；不按价格硬拦，因为运营赠送套餐可 price=0 但额度更高；历史请求/账本不修改）
