...
private-address, or environment-specific Compose overrides. Web defaults to the clean-checkout
multi-stage `apps/web/Dockerfile.web`: its fixed Node 26.4.0/pnpm 11.15.0 builder compiles
Contracts, UI Tokens, and Next standalone inside the image, so the server needs neither Node nor
pnpm and does not depend on prebuilt `.next` or optional untracked `apps/web/public`. `Dockerfile`
