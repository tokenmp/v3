...
- Docker 镜像/部署单元：`tokenmp-v3-auth:<sha>`，构建上下文仓库根目录，Dockerfile 位于 `services/auth/Dockerfile`（按模块自治规范放于模块目录，而非根 `Dockerfile.auth`）；builder 在 `GOWORK=off` 下显式 COPY `packages/go/httpresp` 与 `packages/go/ratelimit`（所有 local replace target 均须在 download/build 层可用）；CI 的 module-autonomy gate 用 `GOWORK=off go build -mod=readonly ./cmd/auth` 防止 workspace 掩盖缺失闭包。最终镜像非 root，不含 migrate CLI，不在启动时执行 migration；运行时镜像**不含 JWT 密钥**，部署必须挂载 key files（见 ADR 0005）。
- Compose 使用实际的 `AUTH_RATE_LIMIT_*` 配置：enabled、外部 Redis address/DB、trusted-proxy CIDRs、各登录/注册/刷新 policy 和 bucket TTL；HMAC 仅以只读 Compose secret 挂载并通过 `AUTH_RATE_LIMIT_HMAC_SECRET_FILE` 读取。Compose 不创建 Redis，也不使用错误别名 `AUTH_REDIS_URL`/`AUTH_HMAC_SECRET_FILE`。Auth `internal/config` 在 `AUTH_RATE_LIMIT_ENABLED=true` 时消费这些变量。
