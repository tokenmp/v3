...
- 负责：TokenMP v3 认证服务的运行时骨架——HTTP server、配置、PostgreSQL 连接、版本化 SQL migration、`/healthz` 与 `/readyz`、优雅关闭、Dockerfile、单元/集成测试、Go CI job。
- 不负责（本 PR 不实现）：注册、登录、JWT 签发、Argon2id 哈希、refresh-token 轮换业务；`preferred_billing`/`fallback_enabled` 等业务字段。
- 所有者：TokenMP 后端基础设施。

## 必读文档

- 模块说明：`README.md`
- 架构决策：`../../docs/adr/0004-auth-service-foundation.md`
...
| `HEAD /healthz` | 同上 | 200；空 body（与 GET 同状态/同 headers，不写 body） | stable | 同上 |
| `GET /readyz` | 已注入 `Pinger`（实现 `Ping(ctx)`） | 200 ok 或 503 `{status:"unready",...}`；**不泄露底层错误** | stable | 同上 |
| `HEAD /readyz` | 同上 | 200 或 503；空 body（不泄露底层错误） | stable | 同上 |

未来能力（注册/登录/刷新）不属于本 PR；不得在文档或代码中描述为已实现。
...
| 依赖 | `gorm.io/gorm` + `gorm.io/driver/postgres` | ORM 与连接池 | Go module import | GORM 公开 API | `go vet`、`go test -race` |
| 依赖 | `github.com/jackc/pgx/v5`（`stdlib`） | 集成测试原生 SQL 校验 | build tag `integration` | pgx `database/sql` driver | `go test -tags=integration` |
| 依赖 | `golang-migrate` CLI | 应用 migration | 外部 CLI，非运行时依赖（CI 用 `-tags 'postgres'` 构建） | `migrate -path migrations` | CI migration 周期 |
...
- 配置和环境变量：仅 `AUTH_*`（见 README 表格）；`AUTH_DATABASE_URL` 必填，严格解析
  ...
  错误不回显 URL/凭据。
- Docker 镜像/部署单元：`tokenmp-v3-auth:<sha>`，构建上下文仓库根目录，Dockerfile 位于 `services/auth/Dockerfile`（按模块自治规范放于模块目录，而非根 `Dockerfile.auth`）；最终镜像非 root，不含 migrate CLI，不在启动时执行 migration。
...
- **DO NOT** 在本 PR 实现登录/JWT/Argon2 —— 本 PR 是 Foundation；后续渐进升级 Argon2id 见 ADR 0004。
...
出现以下变化时同步更新本文件、README、`api/openapi.yaml` 与 ADR 0004：

- 新增/移除对外端点或修改返回结构。
- 引入 login/JWT/Argon2 等业务能力（届时升级 README 与契约）。
- 新增/删除直接依赖或首批真实消费者。
