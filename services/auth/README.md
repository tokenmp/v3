...
- `internal/security/refresh`: 32-byte base64url refresh token generation +
  SHA-256 hashing
- `internal/security/apikey`: `sk-`-prefixed 43-char base62 (0-9A-Za-z) API key encoding 32 random bytes
...
- `migrations/000001_create_users`, `000002_create_auth_sessions`, and
  `000003_create_api_keys` (up/down) — versioned Auth-owned schema; API keys
  unify the legacy `api_keys`, `user_api_keys`, and `bot_keys` tables and store
  only SHA-256 hashes
...
`api_keys` hash uniqueness, lifecycle checks, ownership FK, and updated_at
trigger / `auth_sessions` FKs and unique indexes / the revoke_reason allow-list
and revoked consistency / the INET ip / TEXT user_agent columns / the
