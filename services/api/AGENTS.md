...
After `shared-rate-limit` merges, Compose uses the actual `API_RATE_LIMIT_*` inputs:
...
Compose also mounts the Config admin token as a read-only secret and reserves
`API_CONFIG_SERVICE_TOKEN_FILE`. The `config-publish-hardening` API code currently accepts
only the string `API_CONFIG_SERVICE_TOKEN`; it must add safe `_FILE` consumption before this
Compose setting is enabled in a merged runtime. Do not pass a secret-file path as the string
