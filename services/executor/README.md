...
Responses 与 Images 仍是鉴权后的协议原生 `501`。stream Driver 仍采用 request-local pre-commit retry、一次 quota Reserve 与唯一 Finalize/Release；SDK retry=0、禁止 redirect。transport、composition、stream boundary/facade、SDK 和 process/route tests 现均覆盖此接线。
