...
### Phase 8.2 internal StreamDriver（未接 runtime）
...
已实施内部 `StreamDriver`：组合 scoped-secret `AttemptSession.ExecuteStream`、精确 stream capability registry、sequence-indexed owned-payload source/sink adapter 与 Bridge；仅在 commit 前按冻结 policy retry，一次 Reserve 后仅 Finalize 或 Release，记录安全 attempt result/log，并以 bounded cleanup 处理终态。OpenAI semantic stream 可内部调用；严格递增 source sequence、256 KiB canonical payload 与 Close/drain 清理均受测试覆盖。尚未接 HTTP transport、composition 或 runtime；因此 schema-valid `stream:true` 仍返回 501。它不表示 public/provider E2E、HTTP atomicity 或 wire-attempt proof；按阶段表下一项是 Anthropic stream SDK，之后是 HTTP SSE transport。
