...
- `internal/sdk` 提供仅有 `Complete` 的 shared non-stream port、opaque/redacted `CredentialSecret`、安全 `ClassifiedError` 与 attempt observer；`internal/sdk/openaiadapter` 使用 official `github.com/openai/openai-go/v3` v3.44.0 完成 OpenAI Chat Completions。target/model/secret 全为 per-call，不读取或修改进程环境；严格 validator 接受契约允许的 tools、vision、thinking 字段并拒绝不安全/未知形状。adapter 在最终 transport 边界安全重建 headers，仅保留允许注入和唯一 Bearer auth，SDK retry 固定为 0 且不跟随 redirect。成功只带 status/request ID metadata，失败安全分类为 timeout、transport、protocol 或 HTTP category；TLS `httptest` 已覆盖。
...
- Config compiler、Store、routing Resolver/Plan 与 Adapter Engine 尚未由 runtime config source、reload loop 或 request pipeline 消费；它们是模块内已实施的纯 Go 能力。OpenAI Chat non-stream adapter 同样尚未接入 pipeline/runtime routing，不能构成端到端业务能力。它只安全使用调用方提供的 opaque per-call secret；不解析 credential ref，也不负责 credential resolution/secret injection，该职责仍留给后续阶段。Responses、Images、stream 与 Anthropic provider 均未实施。
