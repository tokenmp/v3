...
- `internal/transport/executorv1api.Adapter` 实现 generated StrictServerInterface：Foundation `New()` 适配器的 `/healthz` 返回 `200`，`GET /v1/models` 经注入 `CatalogProvider` 返回实际 catalog（未注入时回退 501），`/v1/responses` 经注入的 Responses 执行路径执行 non-stream+stream，`CreateImage` 仍 501；DI `NewNonStream(Options{Executor, RequestIDs})` 适配器经注入的 `NonStreamExecutor` 端口执行 non-stream CreateChatCompletion/CreateMessage/CreateImage；Images 在 executor/quota 前严格 normalize，response format 缺省为 `url`，所有 Images 成功/失败均 `Cache-Control: no-store`；nil/typed-nil executor fail-closed 为安全 internal 错误；CreateResponse 经注入的 Responses 执行路径执行。该 adapter 经 runtime composition（`internal/composition`）接入公开 runtime server/路由，同时仍由路由一致性测试与 generated-handler component tests 驱动。
