...
Executor 是 TokenMP v3 的 Mock-first 模型请求执行服务。runtime `main` 在 `net.Listen` 前经 `composition.Build` 组装 immutable snapshot、secret-free config、credential/identity resolver、in-memory runtime/quota/log、completion+stream SDK registry、Runner/StreamDriver、facades、catalog facade、generated hybrid plain+strict handler 与 `AuthMiddleware(CaptureRawBody(...))`。全部 7 条路由已 runtime 实际注册：匿名 health；鉴权 `/v1/models` 返回 snapshot-backed catalog（不再 501）；鉴权 Responses 支持 non-stream 和 `stream:true` SSE；鉴权 Chat/Messages 支持 non-stream 和 `stream:true` SSE，鉴权 Images 为 completion-only non-stream 执行。Phase 10 streaming 使用 bounded structural detection、一次完整 normalization、auth-before-body-read、pre-commit protocol JSON 与 post-commit no fallback；OpenAI 唯一 `[DONE]`、Anthropic native event framing、canonical payload 256 KiB。无 HTTP atomicity、wire proof、跨进程 exactly-once 或 public/provider E2E。

超出单服务部署范围，不实施：durable idempotency/replay、remote quota/credential resolver。`Retry-After` header parsing 已由 Phase 15 实施；public/provider streaming driver 与 streaming transport/composition 已由 Phase 10 实施；Responses 执行已实施（Phase 14）；Images 执行已实施（Phase 11）；配置热重载已实施（Phase 20）：SIGHUP 触发 on-demand reload；可选 stat mtime+size 轮询。Phase 11 已新增 official OpenAI Images legacy `Images.Generate` 的 SDK `Complete` capability：call-local HTTPS target/model/opaque secret、retry=0/no redirects、唯一 Bearer auth；严格 legacy request、缺省且显式 wire `response_format:url`、严格 HTTPS URL/标准 padded base64/usage 成功响应；wire response 16 MiB、单项 decoded base64 10 MiB、aggregate 12 MiB；已注册至 execution registry/composition/HTTP transport，`/v1/images/generations` 已 runtime 启用；不支持 GPT Image 特有参数或 usage quota。已实施 HTTP transport 层（经 composition 接入 runtime）：OpenAI Chat 与 Anthropic Messages non-stream 的 raw-body 捕获（2 MiB）、strict contract normalizer、protocol renderer、`SafeStrictOptions` 与 DI adapter（`NewNonStream`），并由 generated-handler component tests 覆盖；runtime `main` 经 composition 注册全部生成的 7 条路由。已实施内部 shared `sdk` port、官方 `github.com/openai/openai-go/v3` **v3.44.0** 的 OpenAI Chat Completions 非流式 adapter，以及 Phase 11 legacy Images `Images.Generate` capability：每次调用独立校验 HTTPS target、上游 model 与 secret，SDK retry=0、禁止 redirect；对 Chat 请求执行严格契约验证（含 tools、vision 与 thinking 字段），安全注入与唯一 Bearer 鉴权，记录不含 URL/请求/响应/凭据的 attempt observer metadata；成功返回安全 request ID/status metadata，失败将 timeout、transport、protocol 与 HTTP 状态安全分类。TLS `httptest` 覆盖 target/header 隔离、retry/redirect、分类与无泄漏。与其并列，已实施官方 `github.com/anthropics/anthropic-sdk-go` **v1.58.0** 的 Anthropic Messages 非流式内部 adapter：每次调用独立校验 HTTPS target/path prefix、上游 model 与 opaque secret，使用 `WithoutEnvironmentDefaults`、SDK retry=0 且禁止 redirect；最终 transport 仅允许 per-call `x-api-key` 和固定 `anthropic-version`，并重建允许的 header/query。对 Messages 请求及成功响应执行严格 OpenAPI 形状验证，执行层的 target model 与 effective thinking 具有权威性，并覆盖 tools、vision 与 thinking；成功仅返回安全 status/request-ID metadata，失败安全分类为 timeout、transport、protocol 或 HTTP（含 Anthropic 529 overloaded→unavailable，并由 fixture 映射为 429）。TLS `httptest`、请求/响应 fuzz 覆盖 target/header/query/environment 隔离、retry/redirect、分类和无泄漏。内部 non-stream Runner 已实施（经 composition 接入 runtime）：Resolver/Plan owner binding、纯 `Prepare`、per-attempt credential resolve、Adapter Engine、精确 SDK registry 和 official SDK auth compatibility 均在 attempt 路径中组合；它冻结 request-lifetime retry policy、对每次 SDK 调用设置 request timeout、只 Reserve quota 一次并用安全 Terminalizer 选择单一终态，映射已分类失败并记录不含敏感信息的 execution events。Mock/InMemory/fake 测试覆盖该路径。Runner 不提供 wire-attempt proof、跨进程 exactly-once；durable idempotency/replay、remote quota/credential resolver 超出单服务部署范围，不实施。已实施并提交 Executor v1 generated transport（`internal/contract/executorv1/{models,server}.gen.go`）与 `internal/transport/executorv1api` transport adapter（Foundation `New()` health+501 与 DI `NewNonStream` non-stream 执行，含 raw-body capture/normalizer/renderer/strict options），并新增路由契约一致性测试（`internal/server/contract_test.go`）与 generated-handler component tests（`adapter_integration_test.go`）；但运行时 `main` 现已注册全部生成的 7 条路由（经 runtime composition 接入），不再只经 `internal/transport/healthz` 服务 `/healthz`。`check:generated:executor` 是现有 `go-auth` CI job 中必经的新鲜度门禁；同一 job 还运行 generated contract、transport adapter/route/HTTP conformance 的 race tests，以及 composition/config/app/process race 测试，但仍无独立 Executor CI job（这是设计选择而非缺口：`go-auth` job 复用 Go toolchain 覆盖 Executor 全部 race 门禁）。
...
- `internal/transport/executorv1api.Adapter` 实现 generated StrictServerInterface：Foundation `New()` 适配器的 `/healthz` 返回 `200`，`GET /v1/models` 经注入 `CatalogProvider` 返回实际 catalog（未注入时回退 501），`/v1/responses` 经注入的 Responses 执行路径执行 non-stream+stream，`CreateImage` 经注入的 Images 执行路径执行；DI `NewNonStream(Options{Executor, RequestIDs})` 适配器经注入的 `NonStreamExecutor` 端口执行 non-stream CreateChatCompletion/CreateMessage/CreateImage；Images 在 executor/quota 前严格 normalize，response format 缺省为 `url`，所有 Images 成功/失败均 `Cache-Control: no-store`；nil/typed-nil executor fail-closed 为安全 internal 错误；CreateResponse 经注入的 Responses 执行路径执行。该 adapter 经 runtime composition（`internal/composition`）接入公开 runtime server/路由，同时仍由路由一致性测试与 generated-handler component tests 驱动。
...
- HTTP transport（经 composition 接入 runtime）：`CaptureRawBody`（Chat/Messages body 上限 `MaxCapturedBodyBytes` = 2 MiB）与 strict normalizer/renderer 保持 non-stream 合约；`Hybrid` 用 generated plain handler 做 structural stream dispatch，`stream:false` 委托 generated strict adapter，`stream:true` 一次完整 normalize 后进入 stream facade/Driver。SSE sink 要求 flusher；commit 前协议原生 JSON error，commit 后无 JSON fallback；OpenAI `data:`/唯一 `[DONE]`，Anthropic native `event:`/`data:` 无 `[DONE]`。
...
| `GET /metrics` | `200` Prometheus text | `200` | 匿名；`Cache-Control: no-store`；`Content-Type: text/plain; version=0.0.4`；POST → 405；subpath → 404；禁用时 → 404。 |
| `HEAD /healthz` | `200` 无 body | `200` | 同上，无响应体。 |
| `GET /v1/models` | `401` | catalog / `404` / `500` | 经注入 `CatalogProvider` 返回 snapshot-backed 模型目录；空配置返回空列表。 |
| `POST /v1/chat/completions` | `401` | non-stream 或 SSE / `404` | `stream:true` 启用；空配置下在 SSE commit 前返回 OpenAI JSON `404`。 |
| `POST /v1/messages` | `401` | non-stream 或 SSE / `404` | `stream:true` 启用；空配置下在 SSE commit 前返回 Anthropic JSON `404`。 |
| `POST /v1/responses` | `401` | non-stream 或 SSE / `404` | `stream:true` 启用；空配置下在 SSE commit 前返回 OpenAI JSON `404`。 |
| `POST /v1/images/generations` | `401` | `400` / `404` / provider result | Legacy Images non-stream；所有终态响应 `Cache-Control: no-store`。 |
| 未知 `/v1/*` | `401` | `404` | 鉴权拦截；鉴权后 generated router 返回 `404`。 |

该行为由 `internal/composition` 的 composition-level route conformance test（枚举 OpenAPI 契约全部 operation）与 `cmd/executor` 的 process binary test（实际进程启动、health/unauth chat/鉴权空配置 stream chat JSON 404、Messages stream JSON 404 与 501 route、invalid 配置证明未 bind）验证。

### Phase 10 HTTP streaming（已实施）

`stream:true` 的 OpenAI Chat 与 Anthropic Messages 已经 runtime 启用。`internal/transport/executorv1api.Hybrid` 以 generated plain `ServerInterface` 外层分流：`stream:false` 保持进入 generated strict handler 和原有 non-stream adapter；`stream:true` 先作 bounded structural 检测，再只作一次完整 strict normalization，并交给 transport-neutral `internal/stream` / `internal/streamfacade`、`execution.StreamDriver` 和 exact OpenAI/Anthropic `StreamClient` registry。composition 在 `net.Listen` 前同时注册 completion 与 stream capability；缺少任一 capability 的 enabled route 均 fail-closed。

中间件次序不变：`AuthMiddleware(identity)` 在 `CaptureRawBody(generated hybrid handler)` 外层，因此所有 `/v1` 在 body read/decode 前鉴权，`/healthz` 保持匿名。未提交前的 structural/normalization/routing/execution 错误仍是协议原生 JSON（OpenAI 或 Anthropic）；SSE sink 一旦 commit，任何后续错误、取消或写入不确定性均不再追加 JSON fallback。sink 要求 `http.Flusher`，每个 canonical payload 上限 256 KiB；OpenAI 使用 `data: <json>` 且 finish 后唯一 `data: [DONE]`，Anthropic 使用 native `event:` + `data:` framing、没有 `[DONE]`。它不承诺 HTTP atomicity、wire-attempt proof、跨进程 exactly-once 或 public/provider E2E。

Responses non-stream+stream 已 runtime 启用；Images legacy non-stream 已执行。stream Driver 仍采用 request-local pre-commit retry、一次 quota Reserve 与唯一 Finalize/Release；SDK retry=0、禁止 redirect。transport、composition、stream boundary/facade、SDK 和 process/route tests 现均覆盖此接线。
...
# Stat-based polling (every 30s)
export EXECUTOR_CONFIG_RELOAD_INTERVAL=30s
```

### JWT 验证（Phase 19）

Executor 支持两种 identity source：

1. **JWT（首选）**：设置 `EXECUTOR_JWT_PUBLIC_KEY_FILE` 后，Executor 使用 Auth 签发的 Ed25519/EdDSA JWT 进行本地验证，无需连接 Auth 服务。`iss` 默认 `tokenmp-auth`，`aud` 默认 `tokenmp-web`。JWT source 优先于 identityenv。
...
一份无 models/providers/routes 的 secret-free 配置会编译为无业务 route 的 compiled snapshot（initial generation=1）。该配置下进程可健康启动并提供 `/healthz`，但所有 `POST /v1/chat/completions` 与 `POST /v1/messages` 鉴权请求会因 model 未解析而返回 `404`；`/v1/models` 返回空列表，`/v1/responses` 会因 model 未解析而返回 `404`，`/v1/images/generations` 会因 model 未解析而返回 `404`。credential ref 映射可为空 JSON `{}`（无 enabled route 则无 credential 需求），但 identity 映射必须含至少一个 active service/admin identity 以使鉴权请求可成功。
...
# ── 或使用 JWT 验证（Phase 19）──
# 设置公钥文件路径后，JWT 成为首选 identity source
# export EXECUTOR_JWT_PUBLIC_KEY_FILE=/path/to/auth-public-key.pem
# export EXECUTOR_JWT_ISSUER=tokenmp-auth    # 默认值
# export EXECUTOR_JWT_AUDIENCE=tokenmp-web   # 默认值
# JWT 启用时 EXECUTOR_IDENTITY_MAP_JSON 变为 optional
...
curl -s -o /dev/null -w '%{http_code}\n' -H "Authorization: Bearer $EXECUTOR_API_KEY_LOCAL" http://127.0.0.1:8081/v1/models   # 鉴权 → 200（空配置返回空列表）
...
process binary test（实际进程启动：health、unauth chat、鉴权空配置 stream chat JSON 404、Messages stream JSON 404 与 501 route、invalid 配置证明未 bind）：

```bash
go test ./cmd/executor/...
```

测试不需要数据库。`go test ./...` 包含 compiler、Store、fixture、Adapter Engine 与 C01–C27 覆盖。Engine 测试覆盖 strict JSON、全部有限 DSL actions、literal-only header/query、继续拒绝 `ValueRef`、model-bounded thinking、safe response mapping、atomicity/mutation isolation、race 与 fuzz。OpenAI Chat、Phase 11 completion-only legacy Images 与 Anthropic Messages non-stream adapters 的 TLS `httptest` 覆盖 strict validator、per-call target/model/secret、安全注入/鉴权、retry=0、no redirects、success metadata 及 timeout/transport/protocol/HTTP 分类；Images 另覆盖 default wire `url`、strict URL/base64/usage、16 MiB wire/10 MiB item/12 MiB aggregate caps 与本地 fuzz。Anthropic 另覆盖 `WithoutEnvironmentDefaults`、HTTPS target/path prefix、唯一 `x-api-key`/固定 version、thinking authority、tools/vision、529 mapping 和 request/response fuzz。没有独立 Executor CI job；现有 `go-auth` job 在 generated freshness、generated transport/route conformance race tests 之外，还从本目录运行 `go test -race -count=1 ./internal/adapter/... ./internal/snapshot/... ./internal/routing/... ./internal/execution/... ./internal/requestlog/... ./internal/quota/... ./internal/sdk/... ./internal/configsource/... ./internal/credentialenv/... ./internal/identityenv/... ./internal/quarantinebridge/... ./internal/imagecontract/... ./internal/nonstream/... ./internal/nonstreamfacade/... ./internal/authcontext/... ./internal/requestid/... ./internal/streaming/... ./internal/modelcatalog/... ./internal/modelcatalogfacade/...`。该门禁不运行数据库、live provider 或远端 request pipeline；composition route conformance 与 process binary test 覆盖 runtime wiring（全包装 handler 的匿名/鉴权路由行为与实际进程启动/失败）。`./internal/execution/...` 覆盖 internal Runner、registry、terminal 与 request-local retry State，`./internal/requestlog/...` 与 `./internal/quota/...` 覆盖 Runner 使用的日志和 quota ports，`./internal/configsource/...` 覆盖 strict secret-free 文件源与 initial generation=1 bootstrap；SDK HTTP tests 只使用本地 TLS `httptest` server。`./internal/quarantinebridge/...`、`./internal/imagecontract/...`、`./internal/modelcatalog/...` 与 `./internal/modelcatalogfacade/...` 均为独立 package，必须显式加入 CI race package list 才会被覆盖。

## Docker

多阶段 Dockerfile（`services/executor/Dockerfile`）：builder `golang:1.26.5-alpine` + runtime `alpine:3.22`；`GOWORK=off`、`CGO_ENABLED=0`、`-trimpath -ldflags="-s -w"`；交叉编译 `TARGETOS`/`TARGETARCH`；`ca-certificates` + `tini`；非 root user `10001:10001`；`EXPOSE 8081`；`HEALTHCHECK` 用独立 `healthcheck` 二进制（env `EXECUTOR_HEALTHCHECK_ADDR`，默认 `http://127.0.0.1:8081/healthz`，5s timeout）；`ENTRYPOINT` tini + executor；镜像名 `tokenmp-v3-executor`。不 COPY config/secrets/credentials 进镜像，所有运行时配置通过 env + 挂载。
...
```bash
docker build -f services/executor/Dockerfile -t tokenmp-v3-executor:test .
```

### Run

部署时需设置 `EXECUTOR_HTTP_ADDR=0.0.0.0:8081` 以接受容器外请求（默认 `127.0.0.1:8081` 仅监听 loopback）。

```bash
docker run --rm \
  -p 8081:8081 \
  -e EXECUTOR_HTTP_ADDR=0.0.0.0:8081 \
  -e EXECUTOR_CONFIG_FILE=/app/config.json \
  -e EXECUTOR_CREDENTIAL_REF_MAP_JSON='{}' \
  -e EXECUTOR_IDENTITY_MAP_JSON='{"local":{"subject":"local","key_id":"kid-local","role":"service","status":"active","api_key_env":"EXECUTOR_API_KEY_LOCAL"}}' \
  -e EXECUTOR_API_KEY_LOCAL='placeholder-not-a-real-secret' \
  # JWT 验证（Phase 19）：取消注释以启用 JWT 作为首选 identity source
  # -e EXECUTOR_JWT_PUBLIC_KEY_FILE=/app/auth-public-key.pem \
  # -e EXECUTOR_JWT_ISSUER=tokenmp-auth \
  # -e EXECUTOR_JWT_AUDIENCE=tokenmp-web \
  # -v /path/to/auth-public-key.pem:/app/auth-public-key.pem:ro \
  -v /path/to/config.json:/app/config.json:ro \
  tokenmp-v3-executor:test
```

CI 中为 build-only step（`go-auth` job `docker build` step，无 run/push）。

## 契约与边界

- `packages/contracts/openapi/executor/v1.yaml` 是 Executor HTTP 契约的唯一事实来源。运行时 `main` 现经 `internal/composition` 注册全部生成的 7 条路由；generated `Handler`/`StrictHandler` 与 `executorv1api.Adapter`（含 Foundation `New()` 与 DI `NewNonStream`）由 runtime composition 接入，并由路由一致性测试与 generated-handler component tests 驱动。契约中的 OpenAI Chat 与 Anthropic Messages 已在运行时以 non-stream 或 `stream:true` SSE 执行（鉴权），`/v1/models` 经 `CatalogProvider` 返回实际 catalog，`/v1/responses` 经注入的 Responses 执行路径执行 non-stream+stream，`/v1/images/generations` 为 completion-only non-stream 执行。契约媒体安全边界：有限 request 对象关闭（`additionalProperties:false`）；`ChatTool.function.parameters`、`AnthropicTool.input_schema`、`ResponseTool.parameters`、`AnthropicContentBlock.input`、`CreateResponseRequest.text.format.schema` 等 free-form JSON/JSON Schema 字段有意开放（无 `additionalProperties` 约束）以透传任意合法 schema；成功响应的 `ChatMessage`、`ChatContentPart`、`ChatToolCall` 保持可扩展（无 `additionalProperties:false`）以校验 provider extension；image URL 仅接受 bounded HTTPS（scheme https、非空 host、无 userinfo）或带 image MIME 与 `;base64` 的 `data:` URL，HTTP 及其他 scheme 被拒绝，Anthropic `source.media_type` 枚举为 image/jpeg|png|gif|webp，base64 须为标准 padded 且解码后非空、在解码字节上限内。generated models/strict server 随变更提交，位于 `internal/contract/executorv1/` 并供 adapter 与测试使用；现有 `go-auth` job 运行 generated freshness、generated transport/route conformance race tests，以及 `go test -race -count=1 ./internal/adapter/... ./internal/snapshot/... ./internal/routing/... ./internal/execution/... ./internal/requestlog/... ./internal/quota/... ./internal/sdk/... ./internal/configsource/... ./internal/credentialenv/... ./internal/identityenv/... ./internal/quarantinebridge/... ./internal/imagecontract/... ./internal/nonstream/... ./internal/nonstreamfacade/... ./internal/authcontext/... ./internal/requestid/... ./internal/streaming/... ./internal/modelcatalog/... ./internal/modelcatalogfacade/... ./internal/responsecontract/... ./internal/composition/... ./internal/config/... ./internal/app/... ./cmd/executor/...` 的 compiler/snapshot/internal Runner/config source/models catalog/composition race 门禁。该门禁不运行数据库、live provider 或 request pipeline 的远端调用；SDK HTTP tests 只使用本地 TLS `httptest` server；无独立 Executor CI job（这是设计选择而非缺口）。`./internal/quarantinebridge/...`、`./internal/modelcatalog/...` 与 `./internal/modelcatalogfacade/...` 被显式列出是因为它们与 `./internal/routing/...` 是独立 package，routing 的 race pattern 不会自动测试它们。独立 Executor CI job 与远端/真实 provider 集成超出当前部署范围（runtime 路由 wiring 已由 composition route conformance 与 process binary test 验证）。
- Config compiler、Store、routing Resolver/Plan、Adapter Engine 与 retry State 经 runtime composition（`internal/composition`）间接消费；模块内 strict secret-free config 文件源 `internal/configsource`（`LoadFile`/`CompileAndPublishInitial`）经 composition 启动接入；Phase 20 已实施热重载（SIGHUP + 可选 stat mtime 轮询，`internal/configreload`）。retry State 不是 wire attempt gate：其预算记录逻辑 reservation，而 SDK observer 仅在 `RoundTrip` 前记录 transport-attempt observation。OpenAI Chat 与 Anthropic Messages non-stream adapters 经内部 Runner 与 composition 接入 runtime routing（chat/messages 执行）；二者只安全使用调用方提供的 opaque per-call secret，不解析 credential ref，也不负责 credential resolution/secret injection（该职责由 `credentialenv` + composition 承担）。Responses 执行已实施（`POST /v1/responses` non-stream+stream 已 runtime 启用）；legacy Images completion-only non-stream 已接入 runtime；Chat/Messages stream transport/composition 已接线，但不提供 HTTP atomicity、wire-attempt proof 或跨进程 exactly-once。
