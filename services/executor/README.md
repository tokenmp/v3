...
- `internal/transport/executorv1api.Adapter` 实现 generated StrictServerInterface：Foundation `New()` 适配器的 `/healthz` 返回 `200`，`GET /v1/models` 经注入 `CatalogProvider` 返回实际 catalog（未注入时回退 501），`/v1/responses` 返回协议原生 `501` 错误，绝不启动任何 SSE 流；DI `NewNonStream(Options{Executor, RequestIDs})` 适配器经注入的 `NonStreamExecutor` 端口执行 non-stream CreateChatCompletion/CreateMessage/CreateImage；Images 在 executor/quota 前严格 normalize，response format 缺省为 `url`，所有 Images 成功/失败均 `Cache-Control: no-store`；nil/typed-nil executor fail-closed 为安全 internal 错误；CreateResponse 仍 501。该 adapter 经 runtime composition（`internal/composition`）接入公开 runtime server/路由，同时仍由路由一致性测试与 generated-handler component tests 驱动。
...
Responses 仍是鉴权后的协议原生 `501`；Images legacy non-stream 已执行。stream Driver 仍采用 request-local pre-commit retry、一次 quota Reserve 与唯一 Finalize/Release；SDK retry=0、禁止 redirect。transport、composition、stream boundary/facade、SDK 和 process/route tests 现均覆盖此接线。

### 空配置启动

一份无 models/providers/routes 的 secret-free 配置会编译为无业务 route 的 compiled snapshot（initial generation=1）。该配置下进程可健康启动并提供 `/healthz`，但所有 `POST /v1/chat/completions` 与 `POST /v1/messages` 鉴权请求会因 model 未解析而返回 `404`；`/v1/models` 返回空列表，`/v1/responses` 仍为 `501`，`/v1/images/generations` 会因 model 未解析而返回 `404`。credential ref 映射可为空 JSON `{}`（无 enabled route 则无 credential 需求），但 identity 映射必须含至少一个 active service/admin identity 以使鉴权请求可成功。
