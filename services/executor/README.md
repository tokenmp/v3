...
Executor 是 TokenMP v3 的 Mock-first Foundation 模型请求执行服务。已提供 HTTP health、配置加载、优雅关闭、Mock/InMemory ports、配额 reservation 终态状态机，以及模块内 Config compiler、immutable generation-aware snapshot Store、revision-pinned routing Resolver/Plan、stateless pure-Go Adapter Engine 和 request-local、serial、pure-Go retry decision/attempt-budget State；并已实施 user-authorized 的 runtime composition（`internal/composition`）：runtime `main` 在 `net.Listen` 之前经 `composition.Build` 组装 immutable snapshot store、strict secret-free config source、credential/identity env resolver、in-memory runtime/quarantine/quota/execution log、精确 OpenAI/Anthropic SDK registry、Runner、facade、generated strict handler 与 `AuthMiddleware(CaptureRawBody(...))`，启动拒绝不受支持的 enabled SDK/protocol route。生成的 7 条路由均为运行时实际路由：匿名 `GET`/`HEAD /healthz`、鉴权 `/v1/models`·`/v1/responses`·`/v1/images/generations`（501）与鉴权 non-stream `/v1/chat/completions`·`/v1/messages` 执行。Phase 8.2 internal `StreamDriver` is implemented: it composes `AttemptSession`, the exact stream registry, a sequence-indexed owned-payload source/sink adapter and the protocol-neutral Bridge with request-local pre-commit-only retry, one quota Reserve plus a unique Finalize or Release, safe attempt logs/results and bounded cleanup. The OpenAI semantic stream can be called by this internal driver; source sequence and the 256 KiB canonical-payload bound remain enforced. It has no HTTP transport, composition or runtime consumer, so schema-valid `stream:true` stays 501. It does not claim public/provider E2E, HTTP atomicity or wire-attempt proof. Per the phase table, Phase 9 Anthropic internal StreamClient is now implemented; the next phase is HTTP SSE transport plus composition RegisterStream for OpenAI and Anthropic.
...
### Phase 8.2/9 internal streaming（未接 runtime）

已实现内部 shared `sdk.StreamClient`、`StreamSource` 与 `StreamEvent`：事件含 monotonic `Sequence`、safe Meta 与 adapter-owned canonical `Data`；`Data` 固定上限为 256 KiB。official OpenAI Chat `openai-go` `NewStreaming` adapter 每次仅开一个 stream，retry=0、禁止 redirect、per-call Bearer auth，并只返回安全 opening 2xx status/request-ID metadata；它严格 parse/classify chunk，并将 parser 与 bounded no-raw SSE observer 对齐至该 256 KiB 上限，证明精确一个 `[DONE]`；`Close`/cancel 会释放 in-flight read。

已实施内部 `StreamDriver`：组合 scoped-secret `AttemptSession.ExecuteStream`、精确 stream capability registry、sequence-indexed owned-payload source/sink adapter 与 Bridge；仅在 commit 前按冻结 policy retry，一次 Reserve 后仅 Finalize 或 Release，记录安全 attempt result/log，并以 bounded cleanup 处理终态。OpenAI semantic stream 可内部调用；严格递增 source sequence、256 KiB canonical payload 与 Close/drain 清理均受测试覆盖。

Phase 9 已增加 official `anthropic-sdk-go` v1.58.0 Messages internal `StreamClient`：secure opening 使用 HTTPS target/path prefix、execution-authoritative model/thinking、`WithoutEnvironmentDefaults`、retry=0/no redirects、per-call sole `x-api-key`、fixed `anthropic-version` 与 SSE Accept。adapter-owned bounded SSE parser/state（不信任 SDK event object）处理 `message_start`、text/thinking/tool deltas、thinking signature、ping、native error、usage 与 `message_stop`；signature 仅保留在 canonical owned payload 供下游，绝不进入 metadata/log；native error payload 为空，分类 first-wins。尚未接 HTTP transport、composition `RegisterStream` 或 runtime；因此 schema-valid `stream:true` 仍返回 501。它不表示 public/provider E2E、HTTP atomicity 或 wire-attempt proof；下一阶段是 HTTP SSE transport，并在 composition 为 OpenAI 与 Anthropic 注册 StreamClient。
