...
// WithSnapshotResult is a functional option setting the static Snapshot result.
func WithSnapshotResult(s Snapshot) func(*Mock) {
	return func(m *Mock) { m.snapshotResult = s }
}

// WithSnapshotErr is a functional option setting the static Snapshot error.
func WithSnapshotErr(err error) func(*Mock) {
	return func(m *Mock) { m.snapshotErr = err }
}

// WithSnapshotFn is a functional option installing a Snapshot handler.
