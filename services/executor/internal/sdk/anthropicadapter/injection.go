...
// reduce to "apikey"). It does not change matching semantics for
// already-canonical names because no separator survives.
...
// defense-in-depth gate for "prohibit plan x-api-key": the SDK's native
// x-api-key (from WithAPIKey) is the only credential channel.
//
...
func deniedHeader(name string) bool {
	...
	switch {
	case n == "host", n == "content-length", n == "x-api-key", n == "authorization":
		return true
	case strings.HasPrefix(n, "proxy-"), strings.HasPrefix(n, "forwarded"), n == "forwarded":
		return true
	case strings.HasPrefix(n, "x-forwarded-"):
		return true
	case strings.HasPrefix(n, "x-stainless-"):
		return true
	case strings.HasPrefix(n, "anthropic-"):
		return true
	}
...
// sole x-api-key), and a credential-like query name is a strong signal of a
// smuggling attempt or a misconfiguration. This is the defense-in-depth gate
// for "prohibit plan credential-via-query", comparable to the header
// denylist: it normalizes case and alternate punctuation so a denylist cannot
// be bypassed with spellings like Api-Key, api_key, or x-api-key.
//
...
func deniedQuery(name string) bool {
	...
	if alphanumericLower(name) == "authorization" || alphanumericLower(name) == "apikey" {
		return true
	}
...
func validateInjection(plan adapter.InjectionPlan) error {
	for name := range plan.Headers {
		if !rfcToken(name) || deniedHeader(name) {
			return errors.New("anthropicadapter: injected header is denied")
		}
	}
	for name := range plan.Query {
		if !safeSegment(name) || deniedQuery(name) {
			return errors.New("anthropicadapter: injected query name is denied")
		}
	}
	for _, value := range plan.Headers {
		if hasCTL(value) {
			return errors.New("anthropicadapter: injected header value has control character")
		}
	}
	for _, value := range plan.Query {
		if hasCTL(value) {
			return errors.New("anthropicadapter: injected query value has control character")
		}
	}
	return nil
}
