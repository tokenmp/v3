...
var (
	...
	// openai_chat. Non-stream Complete is the only supported path.
	ErrUnsupportedProtocol = errors.New("openaiadapter: unsupported protocol")
	// ErrInvalidBaseURL is returned when call-time base URL validation fails.
	ErrInvalidBaseURL = errors.New("openaiadapter: invalid base url")
	// ErrInvalidRequest is returned when the AppliedRequest body fails strict
	// validation before any network call.
	ErrInvalidRequest = errors.New("openaiadapter: invalid request")
	// ErrMissingUpstreamModel is returned when the forced target model is empty.
	ErrMissingUpstreamModel = errors.New("openaiadapter: missing upstream model")
	// ErrMissingAPIKey is returned when the per-call credential secret is empty.
	ErrMissingAPIKey = errors.New("openaiadapter: missing api key")
	// ErrInvalidInjection is returned when the injection plan contains a denied
	// header/query name, including an Authorization header.
	ErrInvalidInjection = errors.New("openaiadapter: invalid injection plan")
)

// maxSafeTokenBytes bounds the length of a sanitized upstream code/type/id token
