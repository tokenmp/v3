...
现有 `go-auth` CI job 已以 `./internal/sdk/...` 运行该包及 adapter（包括 Phase 11.1 OpenAI Images）的 race tests；无需增加新的 CI package pattern。任何新增 fuzz target 仅本地按需执行，不加入 CI。
...
- **DO NOT** 将 Phase 11.1 Images internal SDK capability 表述为 registry/composition/transport/runtime execution，或改变 `/v1/images/generations` 的鉴权 `501`；它不支持 GPT Image 特有参数或 usage quota。
