// Package streamfacade is the transport-neutral composition root for streaming Executor requests. It implements stream.Executor by pinning the
...
// identifier, and delegating exactly one execution.Run to an injected Driver
// port.
//
...
var (
	...
	ErrMisconfigured = stream.ErrMisconfigured
	...
	ErrRouting = errors.New("streamfacade: routing failed")
	// ErrInvalidProtocol means the request protocol is not one of the supported
	// streaming protocols. It is rendered as a safe internal error.
	ErrInvalidProtocol = errors.New("streamfacade: invalid request protocol")
	...
	ErrReservationID = errors.New("streamfacade: reservation id unavailable")
)

// Driver is the single-call execution port. The facade invokes Run at most
...
// implementation is *execution.Driver; tests inject a double to assert the
// exactly-once contract and to observe the pinned snapshot, resolver, and
// reservation identifier handed to it.
type Driver interface {
	Run(context.Context, execution.StreamInput) (execution.StreamResult, error)
}
...
//
//   - Required (no safe default): Store, Driver, and Quarantine. A nil or
...
// resolver; the Driver resolves credentials per route, so a clean nil surfaces
...
type Options struct {
	...
	// Driver executes exactly one streaming request lifecycle. It is the seam
...
// Facade implements stream.Executor by composing per-request snapshot
// pinning, protocol-filtered routing, trusted-Principal gating, CSPRNG
// reservation identifiers, and a single Driver.Run delegation.
type Facade struct {
	store          *snapshot.Store
	driver         Driver
	credentials    routing.CredentialResolver
...
var _ stream.Executor = (*Facade)(nil)
...
// Execute runs one streaming request. It pins the current snapshot, requires
// and defensively revalidates a trusted Principal, resolves a protocol-filtered
// Plan, issues a reservation identifier, and calls Driver.Run exactly once.
// Every failure path returns a safe sentinel; the zero Result is returned
// alongside any error.
func (f *Facade) Execute(ctx context.Context, req stream.Request) (stream.Result, error) {
	if ctx == nil {
	...
	// upstream call. Store, Driver, and Quarantine are required with no safe
	...
	// AuthNone-only configurations. A typed-nil Driver, Store, Quarantine, or
	// Credentials wrapped in an interface would otherwise panic on dispatch.
	if f == nil || isNilStore(f.store) || isNilDriver(f.driver) || isNilQuarantine(f.quarantine) || isTypedNilCredentialResolver(f.credentials) || isNilProtocolSink(req.Sink) {
		return stream.Result{}, ErrMisconfigured
	}
	// The request protocol selects the route universe. Only the two streaming
	// protocols are admissible; an absent or unknown protocol fails closed
	// rather than resolving cross-protocol routes.
	if !streamProtocol(req.Protocol) {
		return stream.Result{}, ErrInvalidProtocol
	}
	...
	if !validPrincipal(req.Principal) {
		return stream.Result{}, ErrUnauthenticated
	}
	// The trusted request identifier is the only request ID the Driver may
	...
	if err != nil {
		return stream.Result{}, ErrNoSnapshot
	}

	// Build a resolver bound to the pinned snapshot. Its private identity
	// becomes the Plan owner, which the Driver later validates. Quarantine is
	...
	if !requestid.ValidReservationID(reservationID) {
		return stream.Result{}, ErrReservationID
	}

	// Credentials is either a clean nil (admissible for AuthNone-only configs;
	// the Driver resolves per route) or a non-nil resolver: typed-nil was
	// rejected by the misconfiguration gate above, so no normalization is
	// needed and an injected-but-broken value can never reach the Driver.
	...
	// Delegate exactly one Run. The Driver owns Reserve, retry, terminal
	// actions, and safe execution events; it returns only safe errors.
	result, err := f.driver.Run(ctx, in)
	if err != nil {
		return stream.Result{}, err
	}
	return result, nil
...
func validPrincipal(p stream.Principal) bool {
	if p.Status != stream.StatusActive {
		return false
	}
	if p.Role != stream.RoleService && p.Role != stream.RoleAdmin {
		return false
	}
...
// isNilProtocolSink reports whether sink is nil or typed-nil. A streaming
// request without a renderer cannot safely commit a provider stream.
func isNilProtocolSink(sink execution.ProtocolSink) bool {
	if sink == nil {
		return true
	}
...
// isNilDriver reports whether driver is an untyped nil interface or a typed-nil
// value wrapped in the Driver interface.
