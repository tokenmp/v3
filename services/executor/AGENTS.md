...
- 格式化：`gofmt -w .`
- 最小验证：`go mod edit -json`、`go test ./...`、`go build ./...`（包括 `go test ./internal/sdk/...` 的 OpenAI Chat 与 Anthropic Messages TLS/fuzz tests）
- CI compiler/snapshot race 门禁：现有 `go-auth` job 在本目录运行 `go test -race -count=1 ./internal/adapter/... ./internal/snapshot/... ./internal/routing/... ./internal/sdk/...`；覆盖 compiler/snapshot/routing/SDK boundary packages（包括 Engine、OpenAI 与 Anthropic adapters）；不运行数据库、runtime config source、request pipeline 或 live provider，SDK HTTP tests 只使用本地 TLS `httptest` server，且不代表独立 Executor CI job。
...
- **DO NOT** 把 compiler/store/routing/Adapter Engine/retry State 或 OpenAI Chat/Anthropic Messages non-stream SDK adapters 已落地误写为 runtime reload 或端到端业务执行：它们尚无 runtime config source、reload loop、credential resolver/secret injector 或 request-pipeline consumer；response mapping 尤其尚未接入 pipeline。retry State 只管理逻辑 reservation，不能宣称为 wire attempt gate；SDK adapters 是 module-local per-call provider invocation，Responses/Images/streaming 仍未实施。
- **DO NOT** 把 `Plan.Next` 当作 RetryDecider，或在 resolver 外扩大 selector/revision-pinned candidate universe：它只对已冻结候选定义 action scope；retry rule matching 与 attempt budget 由 retry State 管理，但 execution pipeline 仍未实施。
