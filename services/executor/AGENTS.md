...
- **DO NOT** 把 compiler/store/routing/Adapter Engine 或 OpenAI Chat/Anthropic Messages non-stream SDK adapters 已落地误写为 runtime reload 或端到端业务执行：它们尚无 runtime config source、reload loop、credential resolver/secret injector、RetryDecider/attempt budget 或 request-pipeline consumer；response mapping 尤其尚未接入 pipeline。SDK adapters 是 module-local per-call provider invocation，Responses/Images/streaming 仍未实施。
