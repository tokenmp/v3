...
- repository 集成测试隔离约定：每个测试经 `resetSchema`（`DROP SCHEMA public CASCADE`+`CREATE SCHEMA`）+ 全量 up 取得干净完整 schema，cleanup 同样 reset；**绝不用 down migration 做测试清理**（000004 down 是 fail-closed，旧 down-loop 在 down4 RAISE 后忽略错误继续 down3..1 删表会污染下一测试）。`dsn()` 经 `validateTestDSN`（纯函数，libpq 兼容 `pgconn.ParseConfig` 解析）强制 *解析后* `Config.Database == "tokenmp_billing"` 以防 reset 误伤非测试库——**不再用 substring 匹配**（host/user/password/query 含 `tokenmp_billing` 但实际 DB 不同的 DSN 可绕过 substring，PR #215 reviewer P1）；parse 失败/空 DB/不同 DB 均 `t.Fatalf` 稳定 sentinel，错误只说明 expected database name，不回显完整 DSN/密码/host/query。所有破坏性 helper（`applyMigrations`/`resetSchema`/`setupCleanSchemaForDownTest`）均只能经 `dsn(t)` guard 后才 Open/Exec。纯单元测试 `TestValidateTestDSN_*` 覆盖正确 URL/keyword DSN、token 在 host/user/password/query 但 DB 不同均拒绝、空/不同/不同 DB、malformed 与错误稳定不泄漏，无需 PG。`applyUpMigrations` 事后校验 6 张核心表存在以拦截半应用 schema。测试不得依赖执行顺序。
