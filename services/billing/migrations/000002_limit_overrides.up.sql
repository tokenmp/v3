-- =============================================================================
-- TokenMP V3 — Billing DB 迁移 000002：用户套餐配额覆盖（limit overrides）
-- -----------------------------------------------------------------------------
...
-- revoke = 将 effective_until 设为 now()（无需 status 列，软失效）。
-- 设计原则:
...
-- 覆盖查询以 (user_plan_id, scope, kind) 为主，effective_from 用于取 latest reset。
