// Package config loads and validates the config service runtime configuration.
//
...
//
// CONFIG_DATABASE_URL is strictly validated: only postgres/postgresql URLs are
// accepted, must carry a host, a non-empty user and a path of exactly
// /tokenmp_config. Any validation error reports only the failing field, never
...
// Config is the validated config service runtime configuration.
type Config struct {
	HTTPAddr          string
	DatabaseURL       string
	LogLevel          string
...
const (
	...
	// requiredDatabaseName is the only accepted database path. The config
	// service must never connect to any other database.
	requiredDatabaseName = "tokenmp_config"
)

// Sentinel validation errors. None of them embed the URL or credentials.
var (
	ErrMissingDatabaseURL = errors.New("CONFIG_DATABASE_URL is required")
	ErrInvalidDatabaseURL = errors.New("CONFIG_DATABASE_URL is not a valid postgres URL")
)

// Load reads and validates configuration from the environment.
func Load() (Config, error) {
	rawURL := os.Getenv("CONFIG_DATABASE_URL")
	...
	shutdownTimeout, err := getduration("CONFIG_SHUTDOWN_TIMEOUT", defaultShutdownTimeout)
	if err != nil {
		return Config{}, err
	}
	maxOpen, err := getint("CONFIG_DB_MAX_OPEN_CONNS", defaultDBMaxOpenConns)
	if err != nil {
		return Config{}, err
	}
	maxIdle, err := getint("CONFIG_DB_MAX_IDLE_CONNS", defaultDBMaxIdleConns)
	if err != nil {
		return Config{}, err
	}
	connMaxLifetime, err := getduration("CONFIG_DB_CONN_MAX_LIFETIME", defaultDBConnMaxLifetime)
	if err != nil {
		return Config{}, err
	}

	cfg := Config{
		HTTPAddr:          getenv("CONFIG_HTTP_ADDR", defaultHTTPAddr),
		DatabaseURL:       rawURL,
		LogLevel:          strings.ToLower(getenv("CONFIG_LOG_LEVEL", defaultLogLevel)),
		LogFormat:         strings.ToLower(getenv("CONFIG_LOG_FORMAT", defaultLogFormat)),
	...
	if cfg.ShutdownTimeout < 0 {
		return Config{}, fmt.Errorf("CONFIG_SHUTDOWN_TIMEOUT must be >= 0, got %s", cfg.ShutdownTimeout)
	}
	if cfg.DBMaxOpenConns < 0 {
		return Config{}, fmt.Errorf("CONFIG_DB_MAX_OPEN_CONNS must be >= 0, got %d", cfg.DBMaxOpenConns)
	}
	if cfg.DBMaxIdleConns < 0 {
		return Config{}, fmt.Errorf("CONFIG_DB_MAX_IDLE_CONNS must be >= 0, got %d", cfg.DBMaxIdleConns)
	}
...
// validateDatabaseURL parses rawURL and enforces the config service connection
...
//     path exactly /tokenmp_config.
//   - socket form: scheme postgres/postgresql, empty host but a non-empty
//     host= query param (Unix socket path), path exactly /tokenmp_config.
...
func validateLogLevel(level string) error {
	...
	case "debug", "info", "warn", "error":
		return nil
	default:
		return fmt.Errorf("CONFIG_LOG_LEVEL %q must be one of debug|info|warn|error", level)
	}
}
