...
The remaining Admin, Panel, mobile, and specialized specs require an explicitly supplied `BASE_URL` (or protected `E2E_BASE_URL`) and a controlled test environment with provisioned test data. Authenticated user specs receive `E2E_USER_EMAIL`/`E2E_USER_PASSWORD`; Admin specs receive `E2E_ADMIN_EMAIL`/`E2E_ADMIN_PASSWORD`. Optional fixture inputs are `E2E_API_KEY`, `E2E_USER_ID`, and `E2E_PLAN_ID`. Admin and user describes conditionally skip with a clear reason when their required pair is absent, so a credential-free live list/run does not attempt placeholder authentication. Values must remain protected secrets and must never be committed. Some specs exercise data-mutating flows, so they are manual/target-owner responsibilities and are not enabled in regular CI. User-status and role mutation coverage additionally requires a second disposable user distinct from the Admin identity; otherwise it validates the live action affordance without changing the only Admin fixture.

The live suite distinguishes read-only UI coverage from data mutation. Current Admin and combined-flow specs validate current navigation, dialogs, field controls, current Notice/Admin API list paths, and responsive shells without changing shared target data. CRUD, snapshot publication, and identity mutation cases remain present as explicitly reasoned skips until a dedicated disposable fixture is provisioned; they are not silently removed or treated as passes.
