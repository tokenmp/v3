...
- Panel 数据 API：`src/lib/api/user.ts` 已通过 Edge 对接 keys/requests/quota。请求日志列表展示协议/流式、Token（输入/输出/缓存）、TTFT、总耗时、推理速度和 thinking；管理日志额外展示 provider 与有界清洗后的 User-Agent，并由详情 trace 展示完整生命周期。
