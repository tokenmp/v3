...
import { findAdminLabel } from '@/lib/admin-nav';
import {
  DropdownMenu,
  DropdownMenuTrigger,
  DropdownMenuContent,
...
export function AdminHeader() {
  ...
  const currentLabel = findAdminLabel(pathname) ?? 'Admin';
    ...
    <header className="flex h-16 items-center justify-between border-b bg-card px-4 sm:px-6">
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        <nav aria-label="面包屑" className="flex items-center gap-1.5 text-sm min-w-0">
          <span className="hidden sm:inline font-semibold text-foreground/80">Admin</span>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          aria-label="切换主题"
        >
          <Sun className={cn('h-4 w-4 transition-transform duration-300 absolute', theme === 'dark' ? 'rotate-90 scale-0' : 'rotate-0 scale-100')} />
          <Moon className={cn('h-4 w-4 transition-transform duration-300', theme === 'dark' ? 'rotate-0 scale-100' : '-rotate-90 scale-0')} />
        </button>
      ...
      <ConfirmDialog
        ...
        confirmText="退出登录" destructive loading={logoutMutation.isPending}
        onConfirm={() => logoutMutation.mutate()}
      />
      <ConfirmDialog
        open={logoutAllOpen} onOpenChange={setLogoutAllOpen}
        title="登出所有设备" description="此操作将撤销所有设备的登录会话，包括当前设备。你将需要重新登录。确定继续吗？"
        confirmText="确认登出" destructive loading={logoutAllMutation.isPending}
